...
import { createDefaultStageEnemyGroup, type StageDraft } from "./editorApi.ts";
import {
  appendGrid,
  createActionButton,
  createButton,
...
export class StageEnemyEditorStep {
  ...
  private renderContent(): void {
    ...
    if (!editingEnemyGroups) {
      ...
      startActions.appendChild(
        createActionButton("enemyGroups 編集を開始", "editor-btn", () => {
          commitDraft((next) => {
            next.enemyGroups = [];
          }, true);
        })
  ...
  private appendCompositionPreview(
    draft: StageDraft,
    editingEnemyGroups: boolean
  ): void {
    const preview = resolveStageEnemyCompositionPreview(draft as StageDef);
    const usesEnemyGroupsPreview =
      editingEnemyGroups || preview.usesEnemyGroups;
