...
export class BalanceEditorStep {
  ...
  private renderContent(): void {
    ...
    this.container.appendChild(
      createEl(
        ...
        `Lv${PREVIEW_LEVEL} 試算列は Lv1 + 成長 × ${
          PREVIEW_LEVEL - 1
        } に、Lv 時点の自身向けパッシブ stat バフ（満HP想定）を反映します。DPS = floor(ATK) ÷ 実効基本攻撃 interval（2s ÷ SPD 係数 × パッシブ攻撃速度）。基本攻撃のみ。未保存の行は色付きで表示されます。${BALANCE_RANGE_COLUMN_HINT}`
      )
    );
  ...
  private renderRoleGroupedTables(
    ...
    for (const role of BALANCE_ROLE_ORDER) {
      ...
      const section = createEl(
        "section",
        "editor-section editor-balance-role-section"
      );
      ...
      section.appendChild(
        createEl("p", "editor-hint", ROLE_SECTION_HINTS[role])
      );
  ...
  private buildDataRow(
    ...
    ensureClassGrowthFields(cls);
    const growthTier = cls.growthTier!;
    const derived = computeRowDerived(cls, this.options.getSkillRegistry?.());
