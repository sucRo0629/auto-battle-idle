...
describe('operationPassiveAcquireCost (R11c)', () => {
  it('resolves base cost from unlockLevel band table', () => {
    ...
    expect(resolveOperationPassiveBaseCost(catalog, 'mid')).toBe(2);
    expect(resolveOperationPassiveBaseCost(catalog, 'pricey')).toBe(3);
  });
  ...
  it('adds sameClassStackStep per already-acquired count', () => {
    ...
    expect(resolveOperationPassiveAcquireCost(catalog, 'mid', 2)).toBe(4);
    expect(resolveOperationPassiveAcquireCost(catalog, 'pricey', 3)).toBe(6);
  });

  it('treats negative acquired count as zero', () => {
    expect(resolveOperationPassiveAcquireCost(catalog, 'mid', -2)).toBe(2);
  });
  ...
  it('supports zero stack step (legacy flat cost)', () => {
    ...
    expect(resolveOperationPassiveAcquireCost(flat, 'pricey', 5)).toBe(3);
  });
});
