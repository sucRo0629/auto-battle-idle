...
### R11d — 基礎ステの極端化（R5 4 兵科）

Lv 廃止後の基礎帯では、兵科差がステ差分として読めるよう **R5 対象 4 兵科の基礎値を意図的に極端化**する（正本は `data/classes.json`。数値表はここへ転記しない）。

| 方針 | 内容 |
| ---- | ---- |
| 鉄衛士 | 高 HP / 高 DEF、低 ATK |
| 剣術士 | 高 ATK、中〜低 DEF |
| 魔術師 | 低 HP、高 ATK、高 RES、低 DEF |
| 療養師 | 中 HP、低 ATK、高 RES（回復量基準は ATK に依存し得る） |

敵強度は Stage `enemyGroups` の scale で調整（`r10_prototype` は R11d で再設定）。
...
**R4 注記:** 兵科データの責務分離（基礎ステ vs 戦闘方式側）は [combat-data-schema-refactor.md](../plans/combat-data-schema-refactor.md) §3 を正本とする。

| 項目 | 送り先 |
| ---- | ------ |
| 各兵科の基礎 HP / ATK / DEF / RES の具体値 | R5 試作 |
| 各兵科の攻撃間隔（秒） | R5 試作 |
| 各兵科の射程（px） | R5 試作 |
| JSON フィールド名・型 | R5 subset 確定 |
