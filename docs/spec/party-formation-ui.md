...
| 説明文内用語なし              | 辞書登録語はクリック可能 + 用語パネル（§6.4） |
| スキル icon ホバー tooltip のみ | 閲覧カード + 用語パネル                       |
...
13. スキル説明内の辞書登録用語が **クリックで用語パネル** を開く（ホバー説明なし）
14. 用語パネル内の別用語もクリックでき、**履歴 + 戻る** で遷移できる
15. 状態系用語（辞書 `statusCategory`）のパネルに **HUD 同等のアイコン** が表示される
