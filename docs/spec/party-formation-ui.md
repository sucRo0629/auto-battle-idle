...
| 閉じる     | Party Summary ヘッダー右端に `party.backToBattle` を配置（§4.4.5）。secondary 扱い。独立フッター行・モーダル風 primary ボタン・右上 × は **用いない** |
...
| **左ペイン · Party Summary** | 4 つの楕円影を横並びにし、選択済みクラスのキャラ画像を影の上に表示する確認領域。ヘッダー右端に `party.backToBattle`（§4.4.5）。カードリスト・表・手動配置 UI にしない |
...
| 方針 | 内容 |
| ---- | ---- |
| **構造** | Board（左列: Class Select + Party Summary / 右列: Detail）。Formation Screen では独立フッター行を持たない |
| **左ペイン** | Class Select + Party Summary + 戦闘に戻るに **絞る**。クラス概要・スキル詳細は右ペインへ |
...
| **Party Summary** | 左下の 4 影 + キャラ画像 + ヘッダー内 `party.backToBattle`。**画面高さの 16〜21%** | `clamp(132px, 19vh, 182px)` |

Party Summary が Class Select / Detail を圧迫しないこと。右ペイン Detail は Board 高さを確保し、**詳細表示領域** 内でスクロールする。フッター行は持たない。
...
#### 4.4.5 Party Summary ヘッダー / ナビゲーション

| 項目 | 目安 |
| ---- | ---- |
| **配置** | Party Summary ゾーンヘッダー右端（見出し `party.zonePartySummary` の右）。**編成結果に近い位置**へ小さめに配置。独立フッター行・画面下部を占有する大きなボタン行は **持たない** |
| **`party.backToBattle` ボタン** | 補助導線として視認性を保つ。主操作ではない |
| ボタン高さ | **28px 前後** |
| 最小幅 | **104px 前後** |
| font-size | **12px 前後** |
| 左右 padding | **12px 前後** |
...
**注意:** 4 人カード列の横幅を圧迫しないこと（ヘッダー行に配置し、カードグリッドとは分離する）。Web フォーム風の大きな汎用ボタンにはしない。
...
| `summary.ja`                | 短いクラス概要。**2 行 clamp 以上は詰めすぎない**。全文は tooltip で補足可 |
| 主要特徴 / ステータスチップ   | §6.2。役割タグまたは主要特徴として概要帯内に配置 |
...
体験版では、プレイヤーが **実際に使える Lv0 スキル** を優先表示する。

| 項目 | 方針 |
| ---- | ---- |
| 概要内の「主要スキル」 | 体験版で **使用可能なスキルのみ** を対象にする |
| Lv10 / Lv20 スキル | 主要スキルとして **表示しない**。表示する場合はロック表示または将来解放枠として明確に区別 |
| 初回体験版 | Lv10 / Lv20 枠は **非表示を優先** してよい |
| 主要スキルの形式 | **スキル名 + 1 行の役割要約**。倍率・秒数・詳細条件はスキルカード側に任せる |
...
左ペインは **Class Select + Party Summary + 戦闘に戻る** に絞る。クラス概要は右ペインで読む（§6.1）。
...
| 指針 | 内容 |
| ---- | ---- |
| 情報の載せ方 | 左ペイン（Class Select + Party Summary）+ 右ペイン Detail。**札・プレート・楕円影**を盤上に配置 |
...
| 体験版で Lv10/Lv20 を主要スキル表示 | **Lv0 のみ** 優先。未使用スキルは主要スキルに出さない（§6.3） | — |
...
17. 体験版（M1）では **使用可能な Lv0 スキル** を主要スキルとして優先表示し、Lv10/Lv20 を主要スキルとして出さない（§6.3）
...
24. `party.backToBattle` は Party Summary ヘッダー右端の **小さめ補助導線** であり、独立フッター行を占有しない（§4.4.5）
...
| 独立フッター行の `party.backToBattle` | Party Summary ヘッダー右端の小さめ配置（§4.4.5） |
