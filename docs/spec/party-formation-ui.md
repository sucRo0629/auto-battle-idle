...
| 状態系用語 | `statusCategory` があり、かつ `StatusIconRegistry` に PNG が登録されているとき、見出し横に **HUD と同じ状態アイコン** |
...
| ---- | ---- |
| 見出し | 辞書 `title[locale]` |
| 本文 | 辞書 `description[locale]`（あれば。HUD 表示名のみの ID は省略） |
| アイコン | `statusCategory` 指定かつ HUD PNG 登録済みのときのみ |
...
| アタッカー内 | **ファイター / シューター / キャスター** の小見出しで区切る（Kill/Flow 下位系統。[classes-and-skills.md](classes-and-skills.md) §6）。分類：`traits.damageType === magic` → キャスター、物理 + 近接帯 → ファイター、物理 + 遠隔帯 → シューター |
| 不採用   | タブ切替、左サイドバー方式                                                              |
| 配置     | **半透明オーバーレイ上**。モーダルは利用可能な表示領域を広く使い、背面のロスター帯が視界に残る |
| 各行     | `displayName` + `epithetEn` + **`summary.ja`（右列）**                                  |
