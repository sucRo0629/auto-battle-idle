...
- Overview の空き枠クリック → 即 Picker
- Overview の入力済み枠を **もう一度クリック**（選択済み枠の再押下）
- 対象 `partySlotIndex` は選択中インデックスを正とする

**不採用:** Detail の「クラスを変更」ボタン（枠再押下に統合）

### 7.2 閉じ方

- 下部 **決定**（プレビュー中クラスを確定 · 即セーブ）
- 下部 **外す**（`null` メンバー）
- 下部 **戻る**（変更なし）
...
| 項目     | 内容                                                                                    |
| -------- | --------------------------------------------------------------------------------------- |
| 構成     | **左ペイン:** ロール別コンパクト名簿（`displayName` + `epithetEn` のみ） / **右ペイン:** プレビュー中クラスの詳細（ロール + 全文 `summary`） |
| 左ペイン内 | **ディフェンダー / アタッカー / サポーター** の 3 ブロック。アタッカー内はファイター / シューター / キャスターの小見出し（[classes-and-skills.md](classes-and-skills.md) §6） |
| 操作     | 左の行クリックで **プレビュー**（ハイライト）。**決定** で確定 |
| 不採用   | タブ切替、説明付き横長カード縦リスト、右上 ×、一覧行内の長文 `summary` |
| 配置     | **半透明オーバーレイ上の戦術パネル**（`skill-menu-picker-panel`）。背面のロスター帯が視界に残る |
| rangePx  | v1 では **表示しない** |
