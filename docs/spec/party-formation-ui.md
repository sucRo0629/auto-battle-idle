...
- 各クラスの **要約**（`summary.ja`）が選ぶ前に分かること
...
| クラス要約                   | `summary.ja`   | 1〜2 文。ロスター・詳細・Picker 各行に表示（未設定時は非表示） |
...
| タブのみにクラス名            | ロスターカードに epithet・クラス要約           | ✅ |
...
1. 4 人ロスターで各メンバーの **epithetEn**・**クラス要約** が分かる
...
| ロスター直下の人数集計行（v0.4 案） | 各カードで要約が分かるため冗長。v1 非表示 |
