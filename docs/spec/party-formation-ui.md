...
| **クラス要約**              | 編成 UI に表示する 1〜2 文（`summary.ja`）                                                       | `classes.json`（`ClassEditorStep` で編集）                                   |
...
| クラス要約                   | —              | ロスターカードでは **表示しない**（詳細・Picker 各行のみ。未設定時は非表示） |
...
| `summary.ja`                | クラス要約（1〜2 文）。`ClassEditorStep` で編集。未設定時は非表示 |
