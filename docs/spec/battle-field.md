...
### 9.2 プレースホルダ範囲表示（R8 必須）

正式 VFX を待たず、**最小実装時から** プレースホルダ範囲表示を必須とする。

| 項目 | 内容 |
| ---- | ---- |
| 形状 | 円・扇形・帯など、**実際の判定形状と一致** する単純図形でよい |
| 識別 | **味方由来** と **敵由来** を色・線種等で区別できること |
| 重なり | **複数範囲の重なり** が視認できること |
| 確認項目 | 表示範囲と内部判定の一致 / 範囲内外の切り替え / 発生源消滅時の解除 |

**描画層:** `BattleCanvas` 上のフィールド overlay（`battleX` / `spriteDrawY` 基準）。§8.9 の「フィールド上 HP・状態アイコン停止」と矛盾しない — 範囲図形は **ユニット付随 HUD ではなく** 地面 / 発生源起点の overlay とする。

### 9.3 正式 VFX との関係

| Phase | 内容 |
| ----- | ---- |
| **R8** | プレースホルダ図形 + runtime 判定 + 一致検証（テストまたは診断） |
