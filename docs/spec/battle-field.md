...
| 表示切替 | **コンパクト** = 横 4 列（HP・リキャスト・簡易バッジ）。**詳細** = メンバー縦リスト（同一 `.party-hud-panel` 枠）。下のタブで排他切替。別パネルの積み増しはしない |
...
| 要素 | 内容 |
| ---- | ---- |
| 起動 | Party HUD の **クラス名**（`.party-hud-label`）または **アイコン+HP/リキャスト行**（`.party-hud-icon-wrap` / `.party-hud-bars`）へ **マウスオーバー**。詳細モードでも同じ。パネル上にカーソルがあれば表示維持。離れたら非表示 |
