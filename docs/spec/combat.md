...
| 表示 | 場所 | ルール |
| ---- | ---- | ------ |
| **簡易（Party HUD）** | `PartyHudPanel` | 固定 **1 行・5 スロット幅**（最大 4 バッジ + 第 5 枠 `+N`）。クラス名・バッジ行はスロット全幅、24px クラスアイコンは HP/リキャスト行の左（下端揃え）。`overflowCount = max(0, badges.length − 4)`。**全バッジはホバーで表示名 tooltip**（`resolveStatusBadgeTooltipLabel`）。**辞書に `description` があるバッジは加えてクリックで用語パネル**（[party-formation-ui.md §6.4](party-formation-ui.md#64-インライン用語パネル) と同 `GameTermPanel`）。**`+N` 枠のみホバーで省略分の表示名一覧**（`、` 連結） |
