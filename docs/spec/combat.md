...
| **範囲系・オーラ系** — 発生源起点の半径 / 扇形 / 帯内への影響 | **非表示** | 対象全員へ同一アイコンを付けない。フィールド範囲表示を正とする |
...
**未確定（R8 実装前）:** aura 解決の所有モジュール、tick 更新タイミング、プレースホルダ描画の具体 API。データ schema の effect 形状詳細は [combat-data-schema-refactor.md §5](../plans/combat-data-schema-refactor.md#5-作戦内パッシブデータoperation-passive)。
