...
| `threatControl`         | `onDamageTakenFlat?`, `onDamageTakenScale?`, `onBlockFlat?`, `threatDecayMultiplier?`                                                                       | Defender 等の Threat 維持・上昇。被ダメ / ブロック成功時に threat 加算。`threatDecayMultiplier` は tick 減衰倍率（0.5 = 半減速）。被弾 threat の共通ルール代替ではない — クラス固有 passive で明示する                                                                 |
