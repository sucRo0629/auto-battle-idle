...
| `seedFlameOnActiveHit`     | `seedFlameMaxStacks?`, `seedFlameDurationSec?`, `seedFlameDotAtkScale?`, `blazingFlameDotAtkScale?`, `blazingFlameMagicTakenPerStack?`, `blazingFlameMaxStacksDefault?` | 習得者の **active** damage Hit ごとに対象へ種火 +1。basic 非対象。種火 / 熾火 DoT 数値は本 passive で上書き可（未指定は `sorcererFlame.ts` 既定）。実装: `sorcererFlame.ts` |
