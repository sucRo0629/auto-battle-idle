...
攻撃イベントの生成構造と処理対象の違いで役割が分かれる。§2 のアタッカー 3 系統のうち「単体突破・高速処理」の近接側を担う。
...
物理アタッカーは近接・遠隔を問わず、敵の**処理対象と戦闘イベント構造**によって役割分担される職群である。§2 のアタッカー 3 系統のうち「単体突破・高速処理」を中心に構成される（近接＝ファイター、遠隔＝シューター）。
...
前線にバフとデバフを付与し、戦況そのものを操作する制圧型ファイター。
...
罠・範囲 DoT・範囲ノックバックによって戦場を制御し、持続的に敵を削る制圧型遠隔職。
...
遠隔物理の**戦況制圧・空間制御職**。薬草師（HoT + debuff、毒 DoT なし）との差は §クラスヒーラー設計方針。
