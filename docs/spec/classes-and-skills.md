...
※ 物理遠隔 3 クラス（弓術士 / 弩砲士 / 狩猟士）の設計思想・三分類・TBD は **§クラスシューター設計方針** を正とする。
...
シューターは物理遠隔アタッカーであり、射撃行為の**時間構造**を火力に変換する職群。§2 のアタッカー 3 系統のうち「遠距離制圧」軸を担う。

### 設計思想

シューターは「射撃という行為そのものの時間構造」を火力に変換し、攻撃間隔・回数・制圧状態によって戦闘性能が変化する遠隔 DPS 群である。

- 射撃行動の**時間軸**による火力変化
- 単純火力ではなく行動ルール差で役割を分割
- 「撃つ・待つ・仕込む」の 3 軸で構成
- 全クラスが異なる時間設計を持つ遠隔職群
...
`traits.damageType: physical`、`formationRow: back`、射程帯は遠隔物理。

### 弓術士（`at_ranger`・基礎）

#### コンセプト

攻撃速度と攻撃回数を軸に、連続射撃で火力を積み上げる**コンボ型**遠隔 DPS。
...
攻撃間隔と溜め時間を火力に変換し、一撃の重さを極限まで高める**攻城射撃**職。

#### 役割

- 攻撃間隔依存ダメージ設計
- 待機時間を火力資源として利用
- 重撃態勢による SPD 制御（SPD↓ / ATK↑）
- 高 HP 優先・決戦砲撃などの単発特化構造
...
罠・毒・出血・ノックバックによって戦場そのものを制御し、持続的に敵を狩る**制圧型**遠隔職。

#### 役割

- 罠設置によるエリアコントロール
- 出血・毒などの DoT 付与設計
- ノックバック（1.5 秒移動硬直）による行動阻害
- 直接火力ではなく状態異常による削り構造
...
射撃に留まらず、**戦場そのものを制圧する**ハイブリッドハンター。
...
| ------------- | ---------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `at_ranger`   | 連射コンボ | 攻撃回数・ASPD 連動の継続 DPS                       | 溜め・待機依存なし。罠 / DoT 制圧なし                                                                        |
| `at_ballista` | 溜め重撃   | 攻撃間隔・待機を火力資源化。重撃態勢で ASPD↓ / ATK↑ | 連射コンボなし。状態異常・エリア制圧は補助的                                                                 |
| `at_hunter`   | 制圧狩猟   | 罠・DoT・ノックバックによる削りと行動阻害           | 直接 DPS より状態異常と戦場コントロール。薬草師（HoT + debuff、毒 DoT なし）との差は §クラスヒーラー設計方針 |

### 未実装・TBD

- 弩砲士: Lv0 `passive_2` 以降の具体設計
- 狩猟士: 罠設置・DoT・ノックバック（1.5 秒移動硬直）の combat 実装と `data/skills/` への反映
- 3 シューター: Lv10 / Lv20 枝の具体設計

## クラスキャスター設計方針

キャスターは魔法遠隔アタッカーであり、戦闘の**出力・流れ・意味**を操作する職群。§2 のアタッカー 3 系統のうち「戦況変化」軸を担う。
