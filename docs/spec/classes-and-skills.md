...
# 2. Kill / Flow レイヤー（アタッカー戦闘構造）

アタッカー職群の戦闘処理は **Kill（撃破処理）** と **Flow（戦場操作）** の 2 層構造として整理する。重要なのは役割の優劣ではなく、**戦闘ルールへの介入レベル**による分類である。

各クラスの詳細は **§クラス物理アタッカー設計方針**・**§クラスキャスター設計方針** を正とする。
...
| 項目     | Kill Layer       | Flow Layer             |
| -------- | ---------------- | ---------------------- |
| 対象     | 敵 HP            | 戦場ルール             |
| 主目的   | 撃破             | 制御                   |
| 操作対象 | ダメージ量・対象 | 空間・時間・構造       |
| 影響範囲 | 局所             | 戦場全体               |
| 例       | 魔術士・弓術士   | 槍術士・法陣師・狩猟士 |
...
> Kill は「敵 HP をどう削るか」を設計する層である。Flow は「敵をどう倒すか」ではなく「戦闘がどう成立するか」を設計する層である。

---

# 3. アタッカー内部分類

アタッカーは攻撃手段により以下の 3 系統に分化する。
...
攻撃イベントの生成構造と処理対象の違いで役割が分かれる。§3 のアタッカー 3 系統のうち「単体突破・高速処理」の近接側を担う。
...
本ゲームのロール体系は以下の三層構造である。

- 上位ロール：ディフェンダー / アタッカー / ヒーラー
- ディフェンダー内部：鉄衛士 / 護法士 / 闘技士
- アタッカー下位：ファイター / シューター / キャスター
- シューター内部：弓術士 / 弩砲士 / 狩猟士
- キャスター内部：魔術師 / 法陣師 / 符術師

この構造により、
**戦闘の「防御・進行・維持」と「近接・遠隔・魔法」の両軸を明確に分離する。**
...
| ヒーラー(内部は `supporter` のまま) | 回復・支援（後列が典型）                                                |
...
| `at_enchanter` | 符術士 | Enchanter | back  | 遠隔魔法 | —（未実装）                                   | （未実装）           |
| `at_geomancer` | 法陣師 | Geomancer | back  | 遠隔魔法 | —（未実装）                                   | （未実装）           |

※ 物理 6 クラス（剣術士 / 双刃士 / 槍術士 / 弓術士 / 弩砲士 / 狩猟士）の設計思想・三分類・TBD は **§クラス物理アタッカー設計方針** を正とする。

※ 魔法 3 クラス（魔術士 / 符術士 / 法陣師）の設計思想・三分類・TBD は **§クラスキャスター設計方針** を正とする。
...
`formationRow: front`、近接帯。Lv0 / Lv10 / Lv20 の習得パターンは §クラスヒーラー設計方針の「Lv0 / Lv10 / Lv20 習得パターン」（defender 系参照実装）を正とする。
...
| -------------- | -------------------------------------- | ------------------------------------------------- | ----------- |
| `sp_cleric`    | `passive_1` + `passive_2` + `active_1` | **未定**                                          | **未定**    |
| `sp_abjurer`   | `passive_1` + `passive_2` + `active_1` | `passive_3` + `active_2`（双璧の護り）            | `passive_4` |
| `sp_alchemist` | `passive_1` + `passive_2` + `active_1` | `passive_3`（`onDebuffReceived` で dot 1 件解除） | **未定**    |
...
| 味方耐久 UP（`damageTaken`↓ aura） | 結界師 Lv0 `passive_1` のみ可                           | 結界師 Lv10 `passive_3`（全体 barrier）   |
...
物理アタッカーは近接・遠隔を問わず、敵の**処理対象と戦闘イベント構造**によって役割分担される職群である。§2 Kill / Flow の Targeted Kill・§3 のアタッカー 3 系統のうち「単体突破・高速処理」を中心に構成される（近接＝ファイター、遠隔＝シューター）。

### 設計思想

物理アタッカーは「ダメージ量」ではなく、**攻撃イベントの生成構造と処理対象の違い**によって役割が分かれる。
...
ヒット数とコンボ加速によって戦闘密度を上げる高速連撃アタッカー。
...
キャスターは魔法遠隔アタッカーであり、戦闘の**出力・流れ・意味**を操作する職群。§2 Kill / Flow の Fixed / Structured Kill・Structure Flow と §3 のアタッカー 3 系統のうち「戦況変化」軸を担う。
...
対象の条件を読み取り、スキル効果をより有利な形へ**分岐**させる柔軟適応型キャスター。
...
| `at_enchanter` | 条件適応 | 2 系統効果の予測可能分岐・攻撃効率の最適化         | 回復/支援分岐なし。構造は変えず当て方を最適化   |
| `at_geomancer` | 構造操作 | 法陣によるダメージ流量の再配置・味方含む全体最適化 | ATK/DEF buff ではなく分配・転送・集中で損失圧縮 |

### 未実装・TBD

- 符術師: 条件分岐スキルの `fireConditions` / effect 分岐の具体 JSON
...
| `stun`                     | 行動不能（CC）状態にする                                              | `durationSec`（上限 5 秒）                              | 持続時間の長い方を優先。                                            | 通常攻撃・アクティブ発動不可。付与成功時に対象の通常攻撃 CD をリセット。時間 CD は停止。 |
