...
- 単純な **魔法攻撃兵科**
- 優先ターゲット: **最近傍候補**
- **2 戦闘方式** で攻撃形状を変える（方式名・倍率・Hit 数は **未確定**）
- **RES の影響を通常どおり受ける**

**未確定:** 種火 / 熾火を完全廃止するか、MultiLock を使うか、具体 2 方式名、倍率、攻撃間隔。

### 双刃士（`at_assassin`）— 新仕様候補

**兵科固定責務:** 低 HP 対象優先、軽装の処刑・仕留め役。
...
移動阻害・鉄衛士周囲減速は **今回実装仕様に含めない**（R8 作戦内パッシブ候補）。

---

## M1 兵科 — 新仕様候補（R2）

> **案・候補・R5 前に要試作。** 正式なデータ確定表 **ではない**。数値・方式名・Hit 数・射程・攻撃間隔はすべて **未確定**。
