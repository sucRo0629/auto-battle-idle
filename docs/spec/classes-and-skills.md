...
- **preset:** コード正本（`PARTICLE_PRESET_IDS`）。`kind` は `particles` / `ring` / `composite`（拡張可）。初版 `heal_holy_light`（`composite` = 拡散リング + 少数の大きな緑 `+` 上昇）、`spark_burst`（命中・斬撃向けの短時間バースト）。`cross` shape は 1 粒子で縦横両腕を描く。同時 emitter 数・粒子数は Manager 定数で cap。新 preset は `particlePresets.ts` + validate 同期
- **回復系の推奨:** 直接 heal の命中表現は `hitVfx` に `particles` を載せる。`preset: heal_holy_light` と `placement: { anchor: 'footTarget', layer: 'front' }` のように着弾点へ寄せると、PNG strip を主形、粒子を余韻として分離しやすい。
