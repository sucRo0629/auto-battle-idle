...
## 8. 現在地メモ（更新: 2026-07-15・R12b Backend 完了）
...
| **公式次タスク** | **R12c** — 試作作戦 Stage / Wave データ再設計 |

---

## 8b. R11 システム縦切り（2026-07-14）/ 再判定（2026-07-15）

R11 は仕組み（範囲・専用パッシブ枠・資源式・基礎ステ極端化）まで。**Player としての「遊べる」は R12**。

| 確定（仕組み・維持） | 内容 |
| ---- | ---- |
| 取得上限 | なし |
| コスト | `base(unlockLevel) + sameClassCount × stackStep`（固定加算） |
| Wave 資源 | 約 6 人分×1〜2 回強化できる量（体感は R12d） |
...
## 8c. R12 データ再設計（更新: 2026-07-15・R12b Backend 完了）

| ID | 内容 | 状態 |
| -- | ---- | ---- |
| **R12a** | 敵問題・戦術目標・敗因の識別可能性を spec 正本化。勝利条件＝敵全滅 | **完了** |
| **R12b** | **1 Wave 単位の敵問題設計**（成立条件・敵側戦術。現行クラス能力に非依存） | **Backend 完了** / **Player 未達** |
| **R12c** | 試作作戦 Stage / Wave データ再設計 | 未着手（公式次） |
| **R12d** | 手元成立ゲート | 未着手 |
| **R13** | 「繰り返し遊びたいか」 | R12 後 |

旧 R12b 案（CombatModule / 作戦内パッシブデータ再設計）およびクラス能力・敵データは、R12b の敵側戦術・Wave 成立条件を満たすよう **後続フェーズで再設計**する。着手順は R12 後続で確定（R12c と混同しない）。

「繰り返し遊びたいか」は **R13**。R12 完了前に評価しない。
