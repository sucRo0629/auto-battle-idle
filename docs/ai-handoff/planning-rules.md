...
| **公式次タスク** | **R12l** — 作業単位1（新パッシブ経路）実装済み。次は成立確認 → 旧仕様隔離。試作量は本体1 + cost1×3 / 10×2（cost20外）。`fixedCostByPassiveId` 正本 |
...
| ---- | ---- |
| 取得上限 | なし |
| コスト | R12l 4兵科新 path: `fixedCostByPassiveId`（固定）。同兵科加算なし。旧 `unlockLevel` 帯コストは移行待ち |
...
| **R12l** | R5対象4兵科のパッシブ再編（本体1 + cost1×3 / 10×2、全表示。cost20外） | **作業単位1 実装済み / 成立確認・隔離は続き** |
