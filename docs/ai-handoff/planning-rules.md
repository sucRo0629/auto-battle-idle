...
## 8. 現在地メモ（更新: 2026-07-17・R12k 設計開始）
...
| **R12k〜o** | メイン反復構造設計 → R5対象4兵科パッシブ再編 → 4兵科反復試作 → 数値調整 → 手元成立 |
| **公式次タスク** | **R12k** — ローグライク / ローグライトを名称で先決めせず、ゲーム適合性で反復方式、固定クエストとの分離、変動軸、情報開示、1回の境界を設計 |
...
## 8c. R12 データ・反復構造再設計（更新: 2026-07-17）

**設計順（正本）:** R12a〜j の敵問題・module・固定 Stage 技術材料 → **メイン反復構造** → **R5対象4兵科パッシブ再編** → **4兵科反復試作** → 数値調整 → 手元成立 → R13。
...
| **R12k** | メイン反復構造の設計 | **公式次** |
| **R12l** | R5対象4兵科のパッシブ・旧active再編（初期 3 / 2 / 2、全表示） | 未着手 |
| **R12m** | 4兵科だけの反復可能メイン試作 | 未着手 |
...
| 旧 roguelike-mode.md をそのまま復活する | 固定メイン + 副モード、Lv / EXP、旧 Phase 10 前提が現行方針と不一致。R12k で必要要素だけ再設計 |
