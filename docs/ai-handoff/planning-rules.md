...
## 8. 現在地メモ（更新: 2026-07-22・R12m Phase 完了 / 公式次 R12n）
...
| **R12k〜o** | メイン反復構造設計（**R12k 完了**）→ R5対象4兵科パッシブ再編 → 4兵科反復試作（系列 A/B・**R12m 完了**）→ 数値調整（**R12n 未着手**）→ 手元成立 |
| **公式次タスク** | **R12n** — 新構造後の数値強度調整（未着手）。R12m Backend / Player / Phase 完了（実装 HEAD `3f4d628`） |
...
| ---- | ---- |
| 取得上限 | なし |
| コスト | R12l 4兵科新 path: `fixedCostByPassiveId`（固定）。同兵科加算なし。他兵科の旧 `unlockLevel` 帯コストは未移行 |
...
## 8c. R12 データ・反復構造再設計（更新: 2026-07-22）

**設計順（正本）:** R12a〜j の敵問題・module・固定 Stage 技術材料 → **メイン反復構造（R12k 完了）** → **R5対象4兵科パッシブ再編** → **4兵科反復試作（R12m 完了）** → 数値調整 → 手元成立 → R13。
...
| **R12l** | R5対象4兵科のパッシブ再編（本体1 + cost1×3 / 10×2、全表示。cost20外） | **unit3 Backend 完了 / Player 部分完了**（happy-dom。Phase Player 完了ではない） |
| **R12m** | 4兵科だけの反復可能メイン試作（系列 A/B） | **Backend / Player / Phase 完了**（実装 HEAD `3f4d628`）。公式次 **R12n** |
