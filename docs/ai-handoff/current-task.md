...
- **未適用** — `createAlliesFromPartyState` へのマージは R8d
...
**R8d — 戦闘開始注入** — `createAlliesFromPartyState` 前後で operation passive を `learnedPassiveIds` にマージ。最小縦切り: `df_guardian` + `df_guardian_passive_2`。
