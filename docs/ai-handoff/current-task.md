...
## 76. R8c — Wave 間準備 UI + パッシブ取得 API（2026-07-12 完了）

**スコープ:** Wave 間準備画面で作戦内リソースを消費して slot ごとにパッシブを取得する最小導線。戦闘 runtime 注入・Save・専用 JSON・Rank・取り消しは未実装。

**読んだファイル（6 件）:**

| # | ファイル | 目的 |
| - | -------- | ---- |
| 1 | `docs/ai-handoff/current-task.md` | R8c 要件 |
| 2 | `src/game/OperationState.ts` | R8b API・リソース付与拡張 |
| 3 | `src/game/WavePrepScreenHost.ts` | 既存 Wave 準備 UI |
| 4 | `src/game/GameSession.ts` | public API 集約先 |
| 5 | `src/game/wavePrepScreen.test.ts` | テストパターン |
| 6 | `data/skills/passives/df_guardian.json` | 候補 passive 実在確認 |

### 76.1 暫定リソース付与規則

- 中間 Wave クリア後、Wave 間準備 screen 初回突入時に **1 付与**
- `clearedWaveCount` ごとに 1 回のみ（formation 往復では再付与しない）
- 初回出撃前（`clearedWaveCount === 0`）は付与しない
- 取得コストは一律 **1**

### 76.2 候補 passive と対象兵科

| 兵科 | 候補 passive ID | 効果（既存データ） |
| ---- | --------------- | ------------------ |
| `df_guardian` | `df_guardian_passive_2`（立ちはだかる壁） | self + stat buff（def x1.05） |

定数: `src/game/operationPassiveCatalog.ts`

### 76.3 GameSession public API

| API | 役割 |
| --- | ---- |
| `getOperationUnspentResource()` | 残高参照 |
| `getOperationAcquiredPassiveIds(slot)` | 取得済み ID |
| `getOperationPassiveCandidates(slot)` | 兵科に応じた候補 |
| `tryAcquireOperationPassive(slot, passiveId)` | 取得（検証 + 消費 + 追加） |

検証: Wave 準備編集可・slot 有効・候補 pool 一致・`skillRegistry.passives` 実在・未重複・残高 ≥ コスト。失敗時は状態不変。

### 76.4 Wave 準備 UI

- 既存 class / module 選択行に加え、slot ごとに取得済み表示・候補 select・「パッシブ取得」ボタン
- 未確定選択は消費せず、ボタン実行時に `tryAcquireOperationPassive` で即確定
- 画面上部に作戦内リソース残高を表示

### 76.5 checkpoint / retry

- `lastResourceGrantClearedWaveCount` を checkpoint に追加（付与重複防止の復元）
- 次 Wave 確定時に checkpoint へ passives + 残高 + grant 状態を保存
- 現在 Wave 再戦は checkpoint 時点へ復元（未 commit の取得は破棄）
- Wave 0 やり直しは `beginOperation` で全初期化

### 76.6 戦闘 runtime

- **R8d で適用** — `createAlliesFromPartyState` / `prepareAlliesForNextWave` / `syncPartyBuilds` 経由で operation passive を `learnedPassiveIds` へ一時マージ。詳細は §77。

### 76.7 変更ファイル

- 新規: `src/game/operationPassiveCatalog.ts`
- 更新: `OperationState.ts`、`OperationCheckpoint.ts`、`GameSession.ts`、`WavePrepScreenHost.ts`、`wave-prep-screen.css`、上記 4 テスト、`docs/plans/phase-roadmap.md`

### 76.8 テスト

`wavePrepScreen` / `operationState` / `operationCheckpoint` / `operationRetry` — **118 passed**
...
## 77. R8d — 戦闘開始注入（2026-07-13 完了）

**スコープ:** OperationState の slot 別取得済み passive を、味方 Combatant 生成時の `learnedPassiveIds` へ一時マージ。最小縦切り: `df_guardian` + `df_guardian_passive_2`（self DEF x1.05）。

### 77.1 注入経路

`GameSession.getOperationAcquiredPassiveIds` → `BattleEngine` options `getAcquiredOperationPassiveIds` → `createAlliesFromPartyState` / `syncPartyBuilds` → `mergeOperationPassivesIntoBuild`（`learnedPassiveIds` 重複なしマージ）→ 既存 `initBattlePassiveState` / `syncContinuousPassiveAuras`（`passiveBuffBridge` 経由 DEF 反映）。

### 77.2 マージ・検証規則

- `operationPassiveCatalog.isOperationPassiveCandidateForClass` で class-passive 対応を検証
- 不正 ID・別クラス用 passive・registry 未登録 ID は無視
- 既存 `learnedPassiveIds` との重複は追加しない
- 敵 `createEnemyFromTemplate` には非接続
- `PartyMemberState` / Save の `build.learnedPassiveIds` は変更しない

### 77.3 変更ファイル

- 新規: `src/battle/mergeOperationPassivesIntoBuild.ts`、`src/game/operationPassiveInjection.test.ts`
- 更新: `entities.ts`、`BattleEngine.ts`、`GameSession.ts`、`wavePrepScreen.test.ts`

### 77.4 テスト

`operationPassiveInjection` / `wavePrepScreen` / `operationCheckpoint` / `operationRetry` / `passiveBuffBridge` / `passiveEffects` — **113 passed**
...
## 102. R12c — 作戦全体の敵問題の spec 正本化（2026-07-15・Backend 完了 / Player 完了）

### 102.1 目的

R12c で確定済みの「作戦全体の敵問題・Wave 間関係」を、新規設計せず [operation-loop.md §17](../spec/operation-loop.md#17-作戦全体の敵問題r12c) へ正本化した。コード / JSON は変更していない。

### 102.2 読んだファイル

- [current-task.md](current-task.md)（§100〜101）
- [phase-roadmap.md §R12c](../plans/phase-roadmap.md#r12c--作戦全体の敵問題)
- [planning-rules.md](planning-rules.md)
- [operation-loop.md §15 / §16](../spec/operation-loop.md#15-敵問題と戦術目標r12a)

### 102.3 正本化した内容（要約）

| 項目 | 内容 |
| ---- | ---- |
| 作戦全体の敵問題 | 各 Wave の敵問題を Wave 間関係で接続した構造 |
| Wave 間関係 | **継続** / **転換** / **複合** / **対立**。1 作戦に 1 種類以上 |
| Wave 数 | 2 Wave と 3 Wave 以上で成立基準は分けない。3 Wave 以上は高難度 |
| 事前開示 | 全 Wave 敵編成を開示。戦術目標は非表示。推測させる |
| 編成変更 | Wave 間 4 枠すべて変更可。制限・追加コストなし |
| 作戦ポイント | 集中と分散の代償。原則 Wave クリア後付与 |
| 最終 Wave | 通常 Stage は過去 Wave の戦術要求を統合する複合問題（合算ではない） |
| 万能編成 | 全 Stage 共通は不許。特定 Stage 内の同一編成攻略は許容 |
| Stage 先行 | 敵問題 → 能力導出（R12e）。現行能力は採否基準にしない |

### 102.4 変更ファイル

| 箇所 | 内容 |
| ---- | ---- |
| [operation-loop.md §17](../spec/operation-loop.md#17-作戦全体の敵問題r12c) | 作戦全体成立条件・Wave 間関係・不成立例・R12d チェック項目 |
| [spec/README.md](../spec/README.md) | R12c 注記・索引 |
| [enemy-design-concept.md](../enemy-design-concept.md) | §17 へのリンク |
| [phase-roadmap.md §R12](../plans/phase-roadmap.md#r12--試作をゲームにするデータ再設計) | R12c Backend 完了・公式次 R12d |
| [planning-rules.md §8 / §8c](planning-rules.md) | 現在地メモ |
...
| 層 | 結果 |
| -- | ---- |
| Backend | **完了**（設計 Phase）— §17 正本化。production code / JSON 変更なし |
| Player | **完了**（設計 Phase）— 画面変更なし。実 Stage 反映は R12d 以降 |
...
#### 105.5.7 R12g-c5 — 統合 test / debug 検証（Backend 完了・2026-07-16）

**目的:** c2〜c4 の danger targeting と護法士 M2 防護を、実戦経路・damage・対象切替まで統合確認する。新能力は追加しない。

**実施内容:**

- 統合 test: `src/battle/dfPaladinM2.integration.test.ts`
- debug hook: `BattleEngineOptions.onDfPaladinM2ProtectionResult` → `SkillExecutorDeps`（本番 UI / console なし）
- 選定成功時も `dangerSnapshots` を `DfPaladinM2ProtectionResult` へ同梱（選定理由追跡）

**確認済み（Backend）:**

- BattleEngine → SkillExecutor.tryExecute → danger resolver → 防護 overlay → resolveDamage / 実 Hit HP 差分
- danger 順位（current / pending 異敵 / Hit 数 / 最短時刻 / HP tie-break / 決定性）
- 後衛・距離無制限・護法士非移動
- 物理 = 全属性 0.85 / 魔法 = 0.85×0.85
- 切替・同対象 refresh・signal 0・M1 排他・敵味方対称・Wave reset・heal/Barrier/自己防御なし

**Player:** 未完了。既存 CombatModule JSON に M2 ID が無いため画面確認不可。戻し先: **Survival Module JSON 接続後**（R12g 本流）。

**仮実装・data 移管先（固定）:**

| 項目 | 現行所有者 | 最終所有者 |
| ---- | ---------- | ---------- |
| Module ID `df_paladin_mod_danger_guard` | runtime / test 定数 | CombatModule JSON（R12g Survival Module data）。正式 ID 変更時は runtime・test 同時更新。displayName / 配列順に非依存 |
| 全属性 / 魔法追加 multiplier・duration・effect ID | `dfPaladinM2.ts` 仮定数 | CombatModule effect data。validation/editor は **R12g-g**。数値調整は **R12i**。data 接続後に仮定数削除 |
| `TargetSpec kind: danger` | 内部 schema / runtime 接続済み | editor UI は **R12g-g**。editor 前は JSON 入力禁止 |

**R12g-c 全体判定:** **Backend 完了 / Player 未完了**。

**次タスク:** **R12g 本流** — 8 兵科 JSON データ入力（数値は最小）。JSON / editor UI / 正式 UI は本タスクで未変更。

### 105.6 R12g-d1 — 鉄衛士 M1/M2 CombatModule データ再設計（完了・Backend）

**状態:** Backend 完了 / Player 未完了（手元画面確認なし）

**実施内容（要約）:**
- M1 `df_guardian_mod_nearest_strike`: self・物理限定 `damageTaken`（`damageTakenDamageTypes: ["physical"]`）。再付与は同一 skillId+stat で置換
- M2 `df_guardian_mod_guard_focus`: `runtimeEffect.healOnEnemyAttackHpHit.flatAmount` が回復量の所有者。runtime 仮定数削除
- schema: `CombatModuleRuntimeEffect` / `damageTakenDamageTypes`。validation + CombatModule editor 同期
- M1/M2 排他・module 切替で旧 M1 status 除去（`syncIronGuardianModuleStatusEffects`）
...
**次:** **R12g-d5** — Survival CombatModule 共通統合確認

### 105.9 R12g-d5 — Survival CombatModule 共通統合確認（完了・Backend）

**状態:** Backend 完了 / Player 未完了（手元画面確認なし）

**実施内容（要約）:**
- 新規統合 test: `src/battle/survivalCombatModules.integration.test.ts`
- 同一経路で 鉄衛士 / 護法士 / 療養師 / 結界師 の M1/M2 排他・Module 選択・Wave 間切替（Operation API）・支援 no-op・後衛 targeting・敵味方対称・shared TargetSpec 相互非干渉・damage MultiLock 既定・production validate・editor round-trip・description 対比を確認
- runtime / JSON / Stage / Passive / 数値バランスは未変更（今回差分由来の不具合なし）
- 既存 test の legacy 想定を `at_assassin` へ更新（`df_paladin` / `sp_wardweaver` は module 兵科済み）

**Player 未完了の不足:**
- 出撃前 / Wave 間で 4 兵科を編成して M1/M2 差を手元確認していない
- 戻し先: **R12g-d5 Player 確認**（既存 R9.5c SkillMenuPanel / WavePrep 配線は利用可能。大規模 UI 改修は不要）
- 正式 VFX / 専用アイコンは不要
...
### 105.15 R12g-g — validation / authoring 統合（Backend 完了）

**状態:** **Backend 完了** / **Player 未判定**（2026-07-17）。**R12g-d/e Player 未完了**を維持する。authoring 自動テストの成功を、戦闘画面での Player 完了として扱わない。
...
- 全正式対象クラスの M1/M2（現行 registry と class の `combatModuleIds` から導出する **8 兵科 × 各 2 方式 = 16 件**）
- CombatModule editor、保存用 draft API、game-data validation の統合境界

対象クラス・正式モジュール一覧は、既存の正本定数・class data・CombatModule registry から取得する。**8 兵科 ID の重複定数は追加していない。**

**実装内容:**

- 古い 12 件固定前提を、現行 registry / class `combatModuleIds` から導出する **16 件** の正本導出へ更新
- 全正式 CombatModule で次の round-trip を確認:
  - `registry/modules` → `combatModulesDraftFromModules` → `normalizeCombatModulesDraftForSave` → `validateCombatModulesDraftForSave` → `combatModuleFilesFromDraft` → ファイル相当データ再結合 → draft / normalize
- round-trip 前後の正規化済み module 定義が deep equality
- 全正式 module を含む `parseAndValidateGameDataJson(..., { mode: 'editor' })` 成功
- 次の固有フィールドを横断保証:
  ...
  - `TargetSpec.requireBelow`
  - `TargetSpec.excludeRoles`
  - `chainCount`
  - `refillSameTargetOnShortfall`
- 護法士、結界師、弓術士、魔術師の主要入力を editor UI から編集し、draft 反映を確認
- UI テスト用に既存入力へ安定した `data-field` 識別子を追加
- 保存前 validation を、存在する R12g 追加兵科についても 2 件必須に拡張

**validation 境界:**

- `CombatModuleEditorStep` には module 行の全削除 UI がなく、upsert のみ
- R12g 追加兵科の 2 module を外部的に両方 draft から除去した場合、module 配列だけを受け取る `validateCombatModulesDraftForSave` ではその class の存在を推定できない
- この境界はテストで明文化済み。後段の game-data validation が class の `combatModuleIds` との不一致を `Unknown combatModuleId` として拒否することを確認済み

**変更ファイル:**

| ファイル | 役割 |
| -------- | ---- |
| `src/editor/combatModuleAuthoring.integration.test.ts` | **新規** — R12g-g authoring 統合 round-trip / game-data validation |
| `src/editor/combatModuleEditor.test.ts` | editor / draft 回帰・固有フィールド編集 |
| `src/editor/editorApi.ts` | draft API・normalize / validate / 保存前 2 件必須拡張 |
| `src/editor/effectTargetingFields.ts` | 既存 TargetSpec 入力へ `data-field` 識別子追加 |
| `src/editor/formUtils.ts` | フォーム共通ユーティリティ |
| `src/editor/skillEditorCombatFields.ts` | 既存 CombatModule 入力へ `data-field` 識別子追加 |

**対象外（変更なし）:**

- 新しい CombatModule 挙動 / 新 target shape / runtime 戦闘ロジック
- CombatModule ID、表示名、配列順、Phase ID
- パッシブ詳細設計・パッシブ実装 / 作戦内パッシブ authoring 拡張
- 仮数値の再調整（R12i）/ Stage / Wave データ（R12h）
- Player 戦闘画面の確認 / 正式 VFX・i18n・表示 polish

**Backend 完了判定:** 上記 round-trip・game-data validation・固有フィールド横断保証・editor UI draft 反映・保存前 validation 拡張を満たした。**新戦闘仕様・数値・パッシブは追加していない。**

**Player 完了:** **判定しない。** R12g-d/e の Player 確認は未完了のまま。authoring 自動テスト成功を Player 完了とみなさない。

**検証結果（Work 側再実行）:**

| 項目 | 結果 |
| ---- | ---- |
| 関連テスト | Test Files **13 passed** / Tests **264 passed** |
| 対象 | R12g-g authoring 統合、CombatModule editor、editor API、authoring validation preview、CombatModule game-data validation、R12g-d/e クラス別 module、Survival 統合、Kill 統合 |
| `npx tsc --noEmit --pretty false` | リポジトリ全体の既存エラーにより終了コード 2 |
...
- ローグライク / ローグライトを含む方式候補を、名称ではなくゲーム適合性で R12k にて比較する
...
### 105.21 対象網羅・本文選択読解ゲート（2026-07-17）

トークン節約のために確認対象自体を削り、上位設計・関連正本・legacy 矛盾を見落とした再発を防ぐ。

- **読む文章量は減らしてよいが、確認対象の範囲は減らさない**
- 作業前に、上位方針 / spec索引 / 直接依存 / roadmap・handoff / 実体 / legacy の確認対象一覧を作る
- 一覧の全対象へ横断検索し、該当なしも結果として残す
- ヒットした正本節と前提・状態注記は省略せず読む
- 「全文Read禁止」は、そのファイルを確認しない理由にしない
- スコープや前提が変わった場合は確認対象一覧を作り直す
- handoff 各 Phase に、確認済み / 該当なし / 未確認を記録する

正本: [planning-rules.md §2.2](planning-rules.md#22-対象網羅本文選択読解ゲート再発防止必須)、[docs/README.md「JSON の読み方」](../README.md#json-の読み方トークン節約)。
