...
| classId       | 設計の柱                                           | 現行スキル                                                               | v1 確定方針                                                                                                                                                                                                  | 実装影響                                                                                                                                                                     |
| ------------- | -------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `at_warrior`  | 単体安定。高 DEF 単体を DEF 貫通・固定 DPS で処理  | `active_1` 叩き付け、`active_2` 薙ぎ払い。`active_3` / `active_4` 未配置 | **実装済**（Physical pass A）。P3 穿甲 / P4 剛剣の冴え / A3 突き通し / A4 断鉄                                                                                                                               | `ignoredDefBonusDamage` / 貫通フラグ（`pierce*` / `ignoreDamageTakenReduction`）                                                                                             |
...
| `at_warrior`  | Lv10 / Lv20 passive | P3: 15% で DEF 100% 無視。P4: `ignoredDefBonusDamage`                                                                                                               | **実装済**                                                               |
...
| `at_warrior`  | Lv10 active 3       | BAC 7・小前進 + DEF 100% 無視単体（突き通し）                                                                                                                       | **実装済**                                                               |
| `at_warrior`  | Lv20 active 4       | BAC 14・溜め斬り・全軽減貫通（`pierce*` + `ignoreDamageTakenReduction`、回避除く）                                                                                  | **実装済**                                                               |
...
| `at_sigilist`  | Earth / Wind Branch 分岐 + 乾印 / 坤印。条件適応型 Kill   | 旧 JSON 廃棄済み                                                  | 設計は **確定**。combat / JSON / tooling は **Phase 7b 以降**     | `windMark` / `earthMark` state / effect、editor / validate / `formatSkillText` / spec 同期 |
...
| `at_sigilist`  | Lv0 active 1        | Earth / Wind Branch + 乾印 / 坤印 付与・起爆。基本スキル                                                                                                         | 条件分岐 + 対応印 +1 + 1 個起爆（Wind → 乾印、Earth → 坤印）                                                                                                                                                           | 現行 `at_sigilist_active_1`（連印）JSON は **廃棄**。**追加**                                                                                                                                                                                                                  |
| `at_sigilist`  | Lv0 active 2        | 強め Branch + 複数印付与・起爆                                                                                                                                   | 条件分岐 + 対応印 +2 + 2 個起爆                                                                                                                                                                                        | 現行 `at_sigilist_active_2`（爆印）JSON は **廃棄**。**追加**                                                                                                                                                                                                                  |
...
- Caster pass で増やす新要素は、Phase 3 では `at_sorcerer` の既存 effect 整理に限定する。`at_sigilist` / `at_conductor` 向けの乾印 / 坤印（`windMark` / `earthMark`）・damage reservoir 系は **Phase 7b / 8 以降**。
...
5. **印術師・法陣師（Phase 8 以降）**
   - `at_sigilist`: 乾印（`windMark`）/ 坤印（`earthMark`）state / effect、Branch 分岐 tooling、`data/skills/` 投入。
