...
| `at_warrior` | 剣術士 | テストあり。Lv0 目視 polish 済 | 未 |
| `at_assassin` | 双刃士 | 未 | 未 |
...
- [x] 護法陣: `周囲のヘイト下限を自分の72%に引き上げ` / `周囲のヘイト減衰速度低下`
- [x] プレイヤー向け文言から `前列` を廃止（`周囲` に統一。戦闘内部の `formationRow: front` は別）
- [x] Lv10+（慈光 / 真言加護 / 不退転 / 降魔光明）— 共通テンプレのみ適用（本フェーズの目視 polish 対象外）
