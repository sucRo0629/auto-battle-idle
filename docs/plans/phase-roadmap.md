...
**現在地:** **R12b Backend（設計）完了**（1 Wave 敵問題・敵側戦術を `operation-loop.md` §16 へ正本化）。**Player 未達**。公式次は **R12c**（試作作戦 Stage / Wave データ再設計）。詳細は [§R12](#r12--試作をゲームにするデータ再設計) / [current-task.md §100](../ai-handoff/current-task.md)。
...
| **R12** | 試作をゲームにする — 敵問題要件 → 1 Wave 敵問題設計 → 後続データ再設計 | R12a 完了 / R12b **Backend（設計）完了** | R12a 設計完了。R12b **Player 未達** | **R12c へ** |
...
| **R11b** | R5 4 兵科の新仕様作戦内パッシブ候補（枠・専用 ID） | **完了**（各 3・専用 ID）。**効果データ再設計は R12 後続** |
...
| **R12c** | 試作作戦の Stage / Wave 敵・方式配置を要件どおりに再設計 | 未着手（公式次） |
| **R12d** | 手元プレイ合否 — 「ゲームとして遊べる」ゲート（反復欲求の評価はしない） | 未着手 |

**注:** 旧 R12b 案だった CombatModule・作戦内パッシブのデータ再設計は、**R12b（1 Wave 敵問題）の後**に着手順を確定する（R12 後続）。
...
| **R11b** | 作戦専用パッシブ候補枠 | **完了** — 各 3・専用 ID。**効果の中身は R12 後続** |
...
**状態:** **R12b Backend（設計）完了**。公式次 **R12c**。handoff: [current-task.md §100](../ai-handoff/current-task.md)。
...
**順序:** **R12a（完了）→ R12b（Backend 設計完了 / Player 未達）→ R12c → R12d**。旧案の module / passive データ再設計の挿入位置は、R12b 後の着手時に確定する（→ R12 後続。R12c と混同しない）。

### R12a — 敵問題・戦術目標の spec 正本化（完了）
...
| 戦術目標 | 勝利達成のために満たす課題。設計用内部概念。通常プレイでは非表示。停滞時は攻略支援として段階提示可 |
...
### R12b — 1 Wave 単位の敵問題設計（Backend 完了 / Player 未達）
...
| 後続再設計 | クラス能力・CombatModule・作戦内パッシブ・実データは後続。分類採否は現行能力に依存しない |

**Backend 完了:** 設計 Phase — §16 が正本化され、成立条件・敵側戦術 3 分類が参照可能（production code / JSON 変更なし）。

**Player 完了:** **未達**（本 Phase のスコープ外）。プレイヤー画面・実 Wave データでの確認は行わない。ゲームとしてのプレイ成立は **R12c〜R12d**。

**触るファイル（本 Phase）:** `docs/spec/operation-loop.md`、関連索引・handoff。コード / JSON は触らない。

**スコープ外:** 具体クラス再設計、CombatModule / 作戦内パッシブ設計、敵 AI、複数 Wave 作戦全体、Stage 自動生成、UI、数値、`r10_prototype` 修正。

**次:** [R12c](#r12c--試作作戦-stage--wave-のデータ再設計)。

### R12c — 試作作戦 Stage / Wave のデータ再設計

**ゴール:** 試作作戦（既存 `r10_prototype` の更新または後継 ID）の **敵グループ・敵方式・Wave 差** を、確定した敵問題設計に沿って作り直す。

**前提:** [operation-loop.md §15](../spec/operation-loop.md#15-敵問題と戦術目標r12a) / [§16](../spec/operation-loop.md#16-1-wave-単位の敵問題r12b)（1 Wave 成立条件・敵側戦術）。

**Backend 完了:**

- `waves[].enemyGroups` + `selectedCombatModuleId` + scale が設計に追従する
- ステージ詳細で全 Wave の問題差が要約できる（既存 preview 経路）
- 統合テストが新データで完走する

**Player 完了:**

- Wave 間の準備判断が敵問題として読める
- 別の方式 / パッシブ配分でもう一周する理由が敵形から読める

**触る候補:** `data/stages.json`、必要なら敵 scale、`formationHintJa`、統合テスト、[stage-selection-ui.md](../spec/stage-selection-ui.md)。
...
- **R12b Backend（設計）完了。Player 未達。公式次は R12c**（試作作戦 Stage / Wave データ再設計）
