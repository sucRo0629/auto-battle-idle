...
**直近目標:** システム縦切り（R5〜R11）の上に、固定 Stage 1 本ではなく、**繰り返すたびに異なる問題と構築判断が生まれるメイン試作**を成立させる（R12）。ローグライクをメインにする方向を有力案として R12k で設計し、4兵科だけで反復可能な縦切りを作った後に「繰り返し遊びたいか」を評価する（R13）。正式画像・VFX・効果音・i18n・packaging・itch.io 公開は **R13 完了後**に再開する。
...
| **R12k** | **メイン反復構造の設計** — ローグライク主軸案、固定クエストとの分離、変動軸、情報開示、1回の境界 | **次**（doc / 設計） |
...
**ゴール:** 固定 Stage を一度解くだけの試作から、プレイごとに別の問題と構築判断が生まれるメイン試作へ移るための上位仕様を確定する。**ローグライクをメインにする方向を有力案**とするが、旧 [roguelike-mode.md](../spec/roguelike-mode.md) を丸ごと復活させない。
