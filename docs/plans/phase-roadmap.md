...
**直近目標:** システム縦切り（R5〜R11）の上に、固定 Stage 1 本ではなく、**繰り返すたびに異なる問題と構築判断が生まれるメイン試作**を成立させる（R12）。R12k で **シード付き・作者設計の線形 3 Wave 問題系列**を採用確定。次は R12l（4兵科パッシブ再編）→ R12m（系列 A/B の反復試作）→ R12n → R12o の後に「繰り返し遊びたいか」を人間が評価する（R13）。正式画像・VFX・効果音・i18n・packaging・itch.io 公開は **R13 完了後**に再開する。

**現在地:** **R12k 完了（設計）**。公式次は **R12l（R5対象4兵科のパッシブ・旧active再編）**。R12a〜j の固定 `r12_prototype` は技術材料として維持。R13 の反復評価は R12o Player 完了後。正本: [operation-loop.md §21](../spec/operation-loop.md#21-メイン反復構造r12k)。
...
| **R12** | 試作をゲームにする — 敵問題・能力・データ縦切りに加え、**メイン反復構造、4兵科パッシブ再編、反復試作、再調整、手元成立**まで行う | R12a〜j は固定 Stage 技術縦切り。R12k〜o を追加 | R12a〜j だけでは反復可能なゲームの成立証明にならない | **R12l 次** |
...
| **R12l** | **R5対象4兵科のパッシブ・旧active再編** — 各兵科 本体1 + cost 1×3 / 10×2、全候補表示（cost 20 は今回外） | **進行中（unit2 レビュー修正済 / unit3 未着手）** |
| **R12m** | **4兵科だけの反復可能メイン試作** — 系列 A（突破→複数処理）と系列 B（集中被害→分散被害）の 2 問題系列 | 未着手 |
...
**状態:** R12k（メイン反復構造）設計完了。公式次 **R12l**。R12j の成立判定は無効 / 未完了のまま。反復可能なゲームとしての成立ゲートは **R12o**、反復評価は **R13**（開始条件は R12o Player 完了）。正本: [operation-loop.md §21](../spec/operation-loop.md#21-メイン反復構造r12k)。handoff: [current-task.md §105.22](../ai-handoff/current-task.md)。
...
**ゴール:** 旧 `Lv0 / Lv10 / Lv20`、passive / active をそのまま新仕様として扱わず、既存効果を素材として **兵科本体 / Module A / Module B / cost 1 / cost 10 / 廃止候補 / 移行待ち legacy** へ再編する。**R12l unit 1** では `classes.json` の Lv unlock から 4兵科 passives を外し、兵科本体 + 作戦内取得へ切り替える。
...
- 1回の開始から成功 / 失敗 / 結果まで進行できる
- 系列 A と系列 B の materially different な問題系列を seed から選出できる
- 変動結果は seed 等で再現・検証できる
- 固定クエスト用 Stage データと、メイン反復用の入力責務が混ざっていない
- 4兵科以外がなくても R12k の主要判断を試せる
- §21.8 の初期不採用要素を実装しない

**Player 完了:** 複数回開始して、提示問題・取得判断・有効編成のいずれかが実際に変わり、その差を開始前 / 選択前情報から読める。
...
- **公式次は R12l**（R5対象4兵科のパッシブ・旧active再編）。R12k は設計完了（[operation-loop.md §21](../spec/operation-loop.md#21-メイン反復構造r12k)）
- 正式順序は **R12k（完了）→ R12l → R12m → R12n → R12o → R13**
