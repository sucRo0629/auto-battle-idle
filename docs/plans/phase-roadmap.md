...
**Backend 完了（未達）:** 複数 seed / 問題系列で、[operation-loop.md §21.13](../spec/operation-loop.md#2113-r12n-強度比較の-4-検出語候補) の **即全滅候補・無限膠着候補・選択無効候補・単一正解化候補**を検出できる比較と回帰がある。4 語の操作定義は作業単位 1E で固定済み。候補検出は自動不合格ではない。旧表現「即死」は現行定義では「即全滅候補」。
