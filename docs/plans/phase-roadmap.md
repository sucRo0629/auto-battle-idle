...
**現在地:** **R12a 完了**（敵問題・戦術目標を `operation-loop.md` へ正本化）。公式次は **R12b**（1 Wave 単位の敵問題設計）。詳細は [§R12](#r12--試作をゲームにするデータ再設計) / [current-task.md §99](../ai-handoff/current-task.md)。
...
| **R12** | 試作をゲームにする — 敵問題要件 → 1 Wave 敵問題設計 → 後続データ再設計 | R12a 完了 | R12a 完了 | **R12b へ** |
...
**状態:** **R12a 完了**。公式次 **R12b**。handoff: [current-task.md §99](../ai-handoff/current-task.md)。
...
**順序:** **R12a（完了）→ R12b → R12c → R12d**。R12b 以降の細部（module / passive データ再設計の挿入位置含む）は R12b 着手時に確定する。
...
| 戦術目標数 | 1 以上。上限なし |
| 5 要素（目標ごと） | 戦術目標・敗北原因・有効な対処・対処に伴う代償・敗因の識別可能性 |
...
**スコープ外だったもの:** module/passive 数値、新 engine API、正式コンテンツ量産、1 Wave 具体データの設計（→ R12b）。

### R12b — 1 Wave 単位の敵問題設計

**ゴール:** R12a の枠組みに沿い、**1 Wave 単位**で敵問題（戦術目標・5 要素・識別可能性）を設計する。

**Backend / Player 完了条件・触るファイル:** R12b 詳細設計で確定する（本節では仮置きしない）。

**前提:** [operation-loop.md §15](../spec/operation-loop.md#15-敵問題と戦術目標r12a)。

**スコープ外（現時点）:** 複数 Wave 作戦全体の再配置（→ R12c 想定）、正式コンテンツ量産。
...
- **R12a 完了。公式次は R12b**（1 Wave 単位の敵問題設計）
