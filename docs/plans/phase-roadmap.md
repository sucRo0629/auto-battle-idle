...
| **R7** | 反復プレイ — 倍速、Wave 再生 / 再試行、作戦最初からの再試行 | **R7a 完了** → R7b 次 |
...
**R7a 調査完了（2026-07-12）:** [current-task.md §69](../ai-handoff/current-task.md#69-r7a--反復プレイ調査タスク分割2026-07-12-完了)。倍速は未実装（tick gate は `GameSession`）。retry 3 種 API は R6i 済みだが UI は verify 専用 `DebugMenuPanel` のみ。release 敗北は legacy auto-restart。
...
| **R7b** | 倍速 1 / 2 / 4 倍（`GameSession.tick` multiplier） | battleTimeSec が倍率どおり進む |
...
**次タスク:** **R7b** — 倍速 simulation（API + tick テスト。HUD トグルは R7c 以降でも可）。
