...
| **4**  | クラスマスタ + スキル説明 + 編成 UI + **i18n（4e）**；4a **確定済** / 4c **完了** / **4b 完了** / **4d 完了** | **進行中**（**4e / Release M1 準備**） |
...
**開発優先:** **Release M1（体験版）** へ向けて **Phase 4e（英語 i18n）→ Phase 6（体験版ステージ・敵・画面導線・バランス）→ Phase 7（zip ビルド）→ itch.io 公開**。**Release M2** は **Phase 8（本編ステージ・13 クラスバランス）**。体験版と本編のステージ JSON は **別管理**（§Phase 6 / §Phase 8）。ジャンルは **編成解法型オートバトル RPG**。最初の完成単位は **Lv1 章**（Lv10 / Lv20 スキルは実装済みだがコンテンツ・調整は M3 以降）。
...
| Release | 名称 | ゴール | 状態 |
| ------- | ---- | ------ | ---- |
| **M1** | [**体験版**](#m1--体験版) | itch.io 無料公開。Lv1 章 **前半**・解禁 **8 クラス**・**英語 UI** | **準備中**（[Phase 4: 4b・4d 完了 → 4e](phase-4-roadmap.md#release-m1-サマリphase-4-との関係)） |
...
- **4b** — M1 8 クラス Lv0 日本語スキル説明 — **完了**（[phase-4-roadmap.md §4b](phase-4-roadmap.md#4b--スキル説明自動生成日本語--完了2026-06)）
- **4d** — 編成 UI + 統計 UI + HUD — **完了**（2026-06）
- **4e** — 英語 i18n（Release M1 **必須**）— **次**
- **6a** — 体験版に必要な敵テンプレ
...
| **4e**       | **英語 i18n のみ** — i18n 基盤、UI / ゲームデータ / スキル説明の locale 化（**Release M1 必須**。4b 日本語確定済み → 着手可）                                                                                                        | **未着手**                                                 |
...
**4e との順序:** i18n は **Phase 4e で英語のみ**（3 言語目以降はスコープ外）。**4b・4d 完了済み** → 4e 着手（英語は日本語確定文案の翻訳・locale 分岐）。
...
- 4d の編成 UI 骨格が安定していること（文言差し替え先が存在すること）— **充足済み**（2026-06）
- **4b** — M1 8 クラス Lv0 **日本語文案** — **充足済み**（[phase-4-roadmap.md §4b](phase-4-roadmap.md#4b--スキル説明自動生成日本語--完了2026-06)）
...
3. **4e-a** — i18n 基盤 + DOM UI の英語（プレイに必要な最短経路）
4. **4e-b** — `formatSkillText` / クラス名 / 用語辞書 `en` — M1 解禁 **8 クラス**分から
5. M2 前に **13 クラス**分へ拡張
...
```
Phase 4e（英語 i18n。4b M1 Lv0 日本語・4d 編成 UI は完了済み）
    ↓
...
Phase 1 → 2 → 3 → 3d
    ↓
Phase 4（4a 確定 → 4c 完了 → **4b M1 Lv0 完了** → 4d → 4e 英語のみ）
    ↓
