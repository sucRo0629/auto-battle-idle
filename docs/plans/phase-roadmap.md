...
**直近目標:** システム縦切り（R5〜R11）の上に、固定 Stage 1 本ではなく、**繰り返すたびに異なる問題と構築判断が生まれるメイン試作**を成立させる（R12）。ローグライク / ローグライトを含む候補から、このゲームに合う反復方式を R12k で設計し、4兵科だけで反復可能な縦切りを作った後に「繰り返し遊びたいか」を人間が評価する（R13）。正式画像・VFX・効果音・i18n・packaging・itch.io 公開は **R13 完了後**に再開する。

**現在地:** **R12k 設計開始**。R12a〜j で固定 `r12_prototype` の技術縦切りと一部実画面確認までは進んだが、パッシブが旧 Lv 仕様のまま、かつメインの反復構造が未設計だったため、R12j を最終成立ゲートとしては **無効化 / 未完了**とする。公式次は **R12k（メイン反復構造の設計）**。R13 の反復評価は R12o 完了後。
...
| **R12** | 試作をゲームにする — 敵問題・能力・データ縦切りに加え、**メイン反復構造、4兵科パッシブ再編、反復試作、再調整、手元成立**まで行う | R12a〜j は固定 Stage 技術縦切り。R12k〜o を追加 | R12a〜j だけでは反復可能なゲームの成立証明にならない | **R12k 次** |
...
| **R12k** | **メイン反復構造の設計** — ローグライク / ローグライト等の方式比較、固定クエストとの分離、変動軸、情報開示、1回の境界 | **次**（doc / 設計） |
| **R12l** | **R5対象4兵科のパッシブ・旧active再編** — 初期は各兵科 cost 1×3 / 10×2 / 20×2、全候補表示 | 未着手 |
| **R12m** | **4兵科だけの反復可能メイン試作** — 毎回同じ固定 Stage ではない最小縦切り | 未着手 |
...
**状態:** R12a〜j の固定 Stage 技術縦切り後、反復構造とパッシブ再編の欠落を確認。R12j の成立判定は無効 / 未完了。公式次 **R12k**。反復可能なゲームとしての成立ゲートは **R12o**、反復評価は **R13**。handoff: [current-task.md §105.19](../ai-handoff/current-task.md)。
...
**ゴール:** 固定 Stage を一度解くだけの試作から、プレイごとに別の問題と構築判断が生まれるメイン試作へ移るための上位仕様を確定する。ローグライク / ローグライトを含む候補を名称ではなく、**編成問題との相性、判断の読みやすさ、反復時の面白さ**で比較する。旧 [roguelike-mode.md](../spec/roguelike-mode.md) を丸ごと復活させない。

**Backend 完了（設計 Phase）:**

- メインの反復可能な攻略単位と、固定クエスト / チュートリアル / 検証 Stage の責務を分離する
- 何を毎回変えるか（遭遇、Wave 構成、制約、報酬候補、経路等）を決め、少なくとも複数の異なる敵問題を供給できる
- 変動内容の情報開示を決め、盲目的な運試しではなく編成判断になる条件を書く
- シード / 再現性、失敗 / 再試行、1回の長さ、勝利条件、作戦内取得と作戦外解禁の境界を決める
- 分岐マップ、ランダム候補、ショップ、休憩を使うかを個別に決める
- ラン終了時に構築を全消去するか、数値ではない恒久解禁を持たせるかを比較し、ローグライク / ローグライトの呼称は仕様決定後に付ける
- 必要 schema と UI の責務境界だけを定義し、未決の詳細フィールドを先に実装しない

**Player 完了:** 設計 Phase のため、同じ初期条件からでも複数の問題系列が生じ、各選択前に必要情報を読めるプレイ例を文章または図で検証できる。

**スコープ外:** 実装、最終報酬量、正式コンテンツ数、恒久メタの数値。
...
**ゴール:** R12k の上位構造と R12l のパッシブを、R5対象4兵科だけで一周できる最小プレイへ接続する。`r12_prototype` 1本の固定内容を再生するだけでは完了しない。

**Backend 完了:**

- 1回の開始から成功 / 失敗 / 結果まで進行できる
- 同一内容の再生ではなく、少なくとも2つの materially different な問題系列を生成または選択できる
...
R12k〜o の共通スコープ外は、残り11兵科へのパッシブ展開、診断基盤再構築、戦場移動 legacy cleanup、Stage 削除、正式画像 / VFX / 効果音、i18n、packaging、itch.io、作戦途中セーブ。**ローグライク問題生成を一律スコープ外にはしない**。採用範囲は R12k で決め、R12m に必要な最小縦切りを含める。
...
**評価対象先行ゲート:** 発生機構 = R12k のメイン反復構造、設計 = R12k、実装 = R12m、成立 = R12o、評価 = R13。R12l は構築候補、R12n は強度を成立させる補助。R12o Player 未完了なら R13 を開始しない。
...
> 編成、戦闘方式、Wave 間変更、作戦内パッシブを使って、異なる問題が提示されるメイン攻略をもう一度始めたいと思えるか。
...
- **公式次は R12k**（メイン反復構造の設計）。R12a〜j の成果は敵問題・module・Stage・数値の技術材料として再利用する
- 正式順序は **R12k → R12l → R12m → R12n → R12o → R13**
- R12o までは反復評価可能なゲームとして未成立。R13 で初めて「繰り返し遊びたいか」を判定する
