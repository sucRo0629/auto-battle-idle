...
| **R8** | 作戦内パッシブ — リソース消費・任意取得・巻き戻し、**戦闘中表示整理**、**範囲パッシブ runtime 判定 + プレースホルダ範囲描画**（正式 VFX は後続） | **R8c 完了** → R8d 次 |
...
**次タスク:** **R8d** — 戦闘開始注入（operation passive → `learnedPassiveIds` マージ。最小縦切り: `df_guardian` + `df_guardian_passive_2`）。
