...
| **4**  | クラスマスタ + スキル説明 + 編成 UI + **i18n（4e）**；4a **確定済** / 4c **完了** / **4b 完了** / **4d ほぼ完了** | **進行中**（4d 目視確認・**4e / Release M1 準備**） |
...
| Release | 名称 | ゴール | 状態 |
| ------- | ---- | ------ | ---- |
| **M1** | [**体験版**](#m1--体験版) | itch.io 無料公開。Lv1 章 **前半**・解禁 **8 クラス**・**英語 UI** | **準備中**（[Phase 4: 4b 完了 → 4d → 4e](phase-4-roadmap.md#release-m1-サマリphase-4-との関係)） |
...
**4e との順序:** i18n は **Phase 4e で英語のみ**（3 言語目以降はスコープ外）。**4b 日本語確定済み** → 4d 仕上げ後に 4e 着手（英語は日本語確定文案の翻訳・locale 分岐）。
...
- 4d の編成 UI 骨格が安定していること（文言差し替え先が存在すること）— **4d 目視確認中**
...
2. **4d 仕上げ** — 受け入れ条件目視・§11 polish 残確認
...
```
Phase 4d 仕上げ → 4e（英語 i18n。4b M1 Lv0 日本語は完了済み）
    ↓
