...
**現在地:** **R11a〜d はシステム縦切り完了**。ただし R10「試作として遊べる」および R11「プレイアビリティ完了」の Player 判定は **取り消し**（2026-07-15 再判定）。公式次は **R12a**。詳細は [§R12](#r12--試作をゲームにするデータ再設計) / [current-task.md §98](../ai-handoff/current-task.md)。
...
| **R12** | 試作をゲームにする — Stage 反復要件 + module/passive/作戦データの再設計 | 未着手 | 未着手 | **公式次** |
| **R13** | 反復評価 — 「繰り返し遊びたいか」を判断（本来の R10 評価） | R12 後 | R12 後 | 未着手 |

| R11 分割 | ゴール | 状態 |
| -------- | ------ | ---- |
| **R11a** | 効果範囲カテゴリ新仕様化（`pierce` / `multiLock` → §5.7。R5 modules 優先） | **完了**（effectRange + bridge） |
| **R11b** | R5 4 兵科の新仕様作戦内パッシブ候補（枠・専用 ID） | **完了**（各 3・専用 ID）。**効果データ再設計は R12b** |
| **R11c** | 取得レベル基準コスト + 同一クラス積み上げ加重 + Wave 資源（約 6 人分×1〜2） | **完了**（grant 12）。**配分感のデータ調整は R12d** |
| **R11d** | R5 基礎ステ極端化 + `r10_prototype` 強度再調整 | **完了**（初回）。**ゲーム成立線の再調整は R12d** |

| R12 分割 | ゴール | 状態 |
| -------- | ------ | ---- |
| **R12a** | 反復可能な作戦の Stage 要件を spec 正本化し、現行試作とのギャップを明示 | 未着手 |
| **R12b** | CombatModule・作戦内パッシブの **データ再設計**（判断差・挙動差） | 未着手 |
| **R12c** | 試作作戦の Stage / Wave 敵・方式配置を要件どおりに再設計 | 未着手 |
| **R12d** | 手元プレイ合否 — 「ゲームとして遊べる」ゲート（反復欲求の評価はしない） | 未着手 |
...
**達成していないこと:** 上記データを使って「編成・方式・パッシブで敵問題を解く」体験が成立していること。無強化でも惨敗すぎない／Wave 間判断が意味を持つ、は **R12d** で改めてゲートする。
...
| Wave 資源 | クリアごと **約 6 人分×1〜2 回**強化できる量（初期値。配分感は R12d） |
| 基礎ステ | **極端化方針は維持**（数値の再調整は R12d） |
...
| **R11b** | 作戦専用パッシブ候補枠 | **完了** — 各 3・専用 ID。**効果の中身は R12b** |
| **R11c** | Wave 資源と積み上げコスト | **完了** — 式と grant 12。**体感調整は R12d** |
| **R11d** | 基礎ステ極端化 + 初回強度 | **完了** — 初回。**成立線は R12d** |
...
**状態:** **未着手**（公式次。2026-07-15 設計）。handoff: [current-task.md §98](../ai-handoff/current-task.md)。
...
**順序（固定）:** **R12a → R12b → R12c → R12d**。Stage 要件を先に正本化し、判断手段（module/passive）を再設計してから、作戦データへ落とす。強度調整は最後。

### R12a — 反復可能な作戦の Stage 要件（まず）

**ゴール:** 「繰り返し遊べるための Stage 仕様」を authoring 要件として `docs/spec` に正本化し、現行 `r10_prototype` が **どこで要件を満たしていないか** を明示する。

既存から抽出する正本候補（新規システムを創作しない）:

| 出典 | 使う内容 |
| ---- | -------- |
| [design-philosophy.md](../design-philosophy.md) | 敵ギミック × 編成 = 解。クリア後も編成調整サイクルが主 |
| [operation-loop.md](../spec/operation-loop.md) | 各 Wave を独立の編成問題として扱う。Wave 間で方式・パッシブ判断 |
| [enemy-design-concept.md](../enemy-design-concept.md) | 敵は問題提示・教材。個体ではなく編成問題 |
| R10 評価軸（本書 R10） | Wave1→2、方式の挙動差、パッシブが次判断へ、失敗後に別案 |

**Backend 完了:**

- Stage / Wave authoring 要件が [operation-loop.md](../spec/operation-loop.md)（または当該節へのリンクを持つ既存 spec）に書かれている
- `r10_prototype`（または後継作戦 ID）のギャップ表が handoff に残っている

**Player 完了:**

- 要件文書だけでも、「クリア後に再挑戦する意義が無い Stage」を却下できるチェックリストがある
- **このサブフェーズではデータ JSON の全面書き換えは必須としない**（ギャップ明示まで）

**採否の例（要件に含める候補 — R12a で確定）:**

| 含める方向 | 含めない方向 |
| ---------- | ------------ |
| Wave 間で **別軸の問題**（単体圧 vs 線上/複数、耐久壁 vs 火力 等）が読める | 毎ラン敵ランダム（ローグ。試作成立前は凍結路線） |
| 同一作戦に **対抗関係のある複数解**（方式 A 向き / パッシブ B 向き）が設計意図として書ける | 数値スケールだけで「難しい」こと |
| 1 回の「強い一律編成」で全 Wave が溶ける設計を避ける | 旧 demo の線形クリア報酬依存 |

**スコープ外:** module/passive 数値、新 engine API、正式コンテンツ量産。

### R12b — CombatModule・作戦内パッシブのデータ再設計

**ゴール:** R5 4 兵科の **2 方式**と **作戦専用パッシブ各 3** が、同じ敵問題に対して **倍率差ではない判断差** を生む。

**Backend 完了:**

- module / passive JSON が再設計され、validate・既存回帰が維持される
- 各兵科について「方式 A が効く敵形 / 方式 B が効く敵形」と「パッシブ候補の役割分担」が handoff に 1 行ずつ残る
- 新 effect 種別が必要なら同作業で [classes-and-skills.md](../spec/classes-and-skills.md) / editor 同期（skill-data-editor-sync）

**Player 完了:**

- WavePrep / 出撃前 UI で候補を見比べたとき、**誰に・どれを取るか**が敵構成から言語化できる
- 「全部取れば強い」一択、または全候補が同じ役割のステ盛りに見えない

**触る候補:** `data/skills/`（modules / passives の R5 分）、`data/operation-passive-catalog.json`、必要なら `classes.json` の基礎は触らず R12d、editor 同期、最小検証テスト。

**スコープ外:** M1 外兵科、レアリティ、ランダム 3 択、正式 VFX、Stage 敵の全面書き換え（→ R12c）。

### R12c — 試作作戦 Stage / Wave のデータ再設計

**ゴール:** R12a 要件どおりに、試作作戦（既存 `r10_prototype` の更新または後継 ID）の **敵グループ・敵方式・Wave 差** を作り直す。

**Backend 完了:**

- `waves[].enemyGroups` + `selectedCombatModuleId` + scale が要件チェックリストを満たす
- ステージ詳細で全 Wave の問題差が要約できる（既存 preview 経路）
- 統合テストが新データで完走する

**Player 完了:**

- Wave1 の選択が Wave2 の準備判断につながる（敵形を見て言語化できる）
- 「一度クリアしたら終わり」にならない — **別の方式 / パッシブ配分でもう一周する理由**が Stage 文言か敵形から読める
- R12b の module / パッシブ差が、当該作戦で実際に使い分け対象になる

**触る候補:** `data/stages.json`、必要なら敵 scale、`formationHintJa`、`r10PrototypeIntegration` 系テスト、[stage-selection-ui.md](../spec/stage-selection-ui.md) の表示注記。

**スコープ外:** `stages-demo.json` 移行、多数作戦量産、Stage 削除 UI、メタ進行報酬。

### R12d — 「ゲームとして遊べる」ゲート

**ゴール:** データ再設計後の試作を手元で遊び、**惨敗／即溶かし／判断無効**を解消する。ここまでを「試作プレイアビリティ」の Player 完了とする。

**Backend 完了:** 必要なら scale / grant / stackStep / 基礎ステの再調整と回帰。仕組みの新設はしない。
...
- 評価が否定的なら、不足を R12 のどの層（要件 / module-passive / Stage / 強度）へ戻すかを明示する
...
**着手条件:** **R12 で試作がゲームとして成立してから**（R9.5 / R12 と並行しない）。接近・隊形レイヤは `battleLayout` / `partyFormation` / 多数テストと接するため、データ再設計の安定後にまとめて扱う。
...
R12a Stage 反復要件の正本化
  ↓
R12b module / 作戦内パッシブ データ再設計
  ↓
R12c 試作作戦 Stage / Wave 再設計
  ↓
R12d ゲーム成立ゲート（手元）
  ↓
R13 反復評価「繰り返し遊びたいか」
...
- **R11** はシステム縦切り完了。**プレイアビリティ Player は R12**
- **公式次は R12a**
