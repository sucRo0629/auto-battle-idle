...
**現在地:** **R12b Backend（設計）完了**（1 Wave 敵問題・敵側戦術を `operation-loop.md` §16 へ正本化）。**R12c 設計確定**（作戦全体の敵問題・Wave 間関係。spec 正本化は次）。公式次は **R12c**（作戦全体の敵問題の spec 正本化）。詳細は [§R12](#r12--試作をゲームにするデータ再設計) / [current-task.md §101](../ai-handoff/current-task.md)。
...
| **R12** | 試作をゲームにする — Stage 先行の敵問題設計 → 能力導出・分配 → データ実装 → 手元成立 | R12a 完了 / R12b **Backend（設計）完了** / R12c **設計確定** | R12a 設計完了。R12b **Player 未達**。ゲーム成立は **R12j** | **R12c（spec 正本化）へ** |
...
| **R12c** | **作戦全体の敵問題**（Wave 間関係・編成変更・資源・汎用編成の扱い） | **設計確定**（ロードマップ反映済）。**spec 正本化は次** |
| **R12d** | 試作 Stage の**敵問題設計**（具体 Wave / 戦術。JSON 入力はしない） | 未着手 |
| **R12e** | 敵問題から**必要能力・対処能力を導出** | 未着手 |
...
**状態:** **R12b Backend（設計）完了** / **R12c 設計確定**。公式次 **R12c（spec 正本化）**。handoff: [current-task.md §101](../ai-handoff/current-task.md)。
...
R12a 敵問題・戦術目標の基本定義（完了）
  ...
  → R12c 作戦全体の敵問題（設計確定 → spec 正本化）
  → R12d 試作 Stage の敵問題設計（具体・先に）
  → R12e 必要能力・対処能力の導出
...
### R12c — 作戦全体の敵問題（設計確定 / spec 正本化は次）

**状態:** **設計確定**（2026-07-15・ロードマップ反映）。**Backend（spec 正本化）未着手**。正本候補: `operation-loop.md` 新節（§17 相当）。本文の詳細追記は本再編では行わない。
...
**Backend 完了:** 設計 Phase — 上記が `operation-loop.md`（または同等の spec）へ正本化され、R12d 以降が参照できる（production code / JSON 変更なし）。

**Player 完了:** 設計 Phase として完了扱い（画面変更なし）。実 Stage での確認は R12d 以降。
...
### R12d — 試作 Stage の敵問題設計

**ゴール:** R12a〜c に沿い、試作作戦（既存 `r10_prototype` の更新または後継）の **各 Wave の敵問題・敵側戦術・Wave 間関係** を具体設計する。**Stage 先行** — 能力や JSON より先に問題を書く。

**Backend 完了:**

- 試作 Stage について、Wave ごとの戦術目標・敵側戦術・Wave 間関係・5 要素が文書化されている（handoff または spec 追記）
- R12c の Wave 間関係を 1 種類以上含む
- JSON / class データはまだ変更しない（設計のみ可）

**Player 完了:** 設計 Phase として完了（プレイ確認は R12j）。

**触る候補:** handoff、必要なら `operation-loop.md` の例示節、`docs/enemy-design-concept.md`。`data/stages.json` の本入力は **R12h**。

**スコープ外:** 必要能力の導出（→ R12e）、module/passive 分配（→ R12f）、データ実装（→ R12h）、数値調整（→ R12i）。

### R12e — 敵問題から必要能力・対処能力を導出

**ゴール:** R12d で書いた敵問題に対し、**プレイヤー側に必要な対処能力**と**敵側に必要な能力**を一覧化する。兵科への割当・JSON 入力はしない。

**Backend 完了:** 対処能力 / 敵能力の導出表が handoff または設計 doc にあり、R12d の各 Wave と対応している。

**Player 完了:** 設計 Phase として完了。

**スコープ外:** 兵科・module・passive への割当（→ R12f）、データ入力（→ R12g/h）。
...
- **公式次は R12c（作戦全体の敵問題の spec 正本化）**。その後 R12d〜j → R13
