...
**現在地:** **R12m 1B / 1C Backend 完了** + **Player production DOM 自動経路成立**（happy-dom。実ブラウザ未確認）。2Q1 監査で production 不足（Wave 間 §21.5 開示・4 兵科 Player 編成制限）。R12m Phase 全体は未完了。**公式次は R12m 実ブラウザ確認**（系列 A 完走・結果・同 seed / 新 seed 再開始の手動確認）。R12n / R12o / R13 開始条件未達。R12l は unit3 Backend 隔離完了（Player は部分のみ）。正本: [operation-loop.md §21](../spec/operation-loop.md#21-メイン反復構造r12k)。handoff: [current-task.md §105.28](../ai-handoff/current-task.md)。
...
**評価 Phase の追加・開始条件:** [planning-rules.md §2.1「評価対象先行ゲート」](../ai-handoff/planning-rules.md#21-評価対象先行ゲート再発防止必須)を必須とする。評価したい性質について、**発生機構の設計 → 実装 → 手元成立 → 評価**の4段階と Phase ID が揃わないロードマップは未成立として差し戻す。
...
| **R12** | 試作をゲームにする — 敵問題・能力・データ縦切りに加え、**メイン反復構造、4兵科パッシブ再編、反復試作、再調整、手元成立**まで行う | R12a〜j は固定 Stage 技術縦切り。R12k〜o を追加 | R12a〜j だけでは反復可能なゲームの成立証明にならない | **R12m 進行中** |
...
| **R12g** | class / module / passive の**データ再設計**（入力） | R12g-g Backend 完了（validation / authoring 統合）。d/e Player 未完了 |
| **R12h** | Stage / Wave **データ実装**（問題構造の JSON 化） | **Backend 完了 / Player 未判定** |
| **R12i** | **数値強度調整**（scale / grant / stackStep / 基礎ステ等） | **Backend初回完了 / Player未判定** — cost `1 / 10 / 20`・grant `[0, 12, 12]`・stackStep `1`。敵scale等は据え置き |
...
| **R12m** | **4兵科だけの反復可能メイン試作** — 系列 A（突破→複数処理）と系列 B（集中被害→分散被害）の 2 問題系列 | **1B / 1C Backend 完了** + **Player production DOM 自動経路成立**（happy-dom。実ブラウザ未確認）。2Q1 production 不足あり。Phase 未完了。実ブラウザ確認待ち。R12n / R12o 未着手 |
...
**2026-07-17 追加判定:** ここで必要なのは固定問題に複数解を持たせることだけではなかった。メインが何度も遊ぶ場であるなら、**プレイごとに別の問題を供給する仕組み**を評価より前に設計・実装する必要がある。R10 と旧 R12j は、反復性を生む仕組みが無いまま反復評価へ進もうとした同根の不具合として、R12k〜o で順序を修正する。
...
**状態:** **R12m 進行中** — 1B / 1C Backend 完了 + Player production DOM 自動経路成立（happy-dom。実ブラウザ未確認）。2Q1 監査で production 不足あり。R12m Phase 未完了。R12n / R12o 未着手。R13 開始条件未達。R12l unit3 Backend 隔離完了（Player 部分のみ）。R12j 成立判定は無効 / 未完了のまま。反復可能なゲームとしての成立ゲートは **R12o**、反復評価は **R13**（開始条件は R12o Player 完了）。正本: [operation-loop.md §21](../spec/operation-loop.md#21-メイン反復構造r12k)。handoff: [current-task.md §105.28](../ai-handoff/current-task.md)。
...
R12a 敵問題・戦術目標の基本定義（完了）
  ...
  → R12f 兵科・CombatModule・作戦内パッシブへの分配（完了）
  → R12g class / module / passive データ再設計
  → R12h Stage / Wave データ実装
  → R12i 数値強度調整（固定 Stage characterization）
  → R12j 固定 Stage 手元ゲート（判定無効 / 未完了）
  → R12k メイン反復構造の設計（完了）
  → R12l R5対象4兵科のパッシブ・旧active再編
...
**責務分離の原則:** 敵問題設計・能力導出・分配設計・データ入力・反復構造・パッシブ再編・数値調整・手元評価を **1 タスクにまとめない**。反復構造を作る前の固定 Stage 数値は最終値にせず、新構造後に R12n で本調整する。
...
**次:** [R12f](#r12f--必要能力を兵科combatmodule作戦内パッシブへ分配完了)。

### R12f — 必要能力を兵科・CombatModule・作戦内パッシブへ分配（完了）

**状態:** **完了**（2026-07-16・設計 Phase）。正本: [operation-loop.md §20](../spec/operation-loop.md#20-必要能力の兵科combatmodule作戦内パッシブ分配r12f) / [classes-and-skills.md §R12f](../spec/classes-and-skills.md#r12f--必要能力の分配確定)。
...
| 項目 | 内容 |
| ---- | ---- |
| 対象 | 8 兵科のみ（鉄衛士・護法士・剣術士・双刃士・弓術士・魔術師・療養師・結界師） |
| 上位分類 | Kill / Flow / Survival 維持。A〜G は能力カテゴリ |
| 主担当 | A=剣術士 / B1=双刃士 M1 / B2・F=弓術士 / C=魔術師 M1 / D=護法士 M2・療養師 M1・結界師 M1 / E=護法士 M1・療養師 M2・結界師 M2 / G=編成全体 |
| 代替 | A または B1 / C または D。B1/B2 分離維持 |
| 鉄衛士 | D/E 直接担当外。M1 物理堅守 / M2 不屈（敵 Attack Hit の実 HP ダメージごと自己回復） |
| 直接ダメージ | 上位概念を §20.5 に記録。combat.md 編入・schema は R12g 以降 |
| 能力セット | 正面突破型・後方攻略型・分担安定型いずれも成立 |
| 万能化防止 | Passive で優先ターゲット変更禁止、M1/M2 同時完成禁止、分類反転禁止 等 |

**Backend 完了:** 設計 Phase — §20 / §R12f 正本化。R12g へ渡せる。**production code / JSON / test 未変更。**

**Player 完了:** 設計 Phase として **完了**（画面・データ・手元プレイ確認なし）。

**スコープ外だったもの:** 効果数値の最終化（→ R12i）、Stage JSON（→ R12h）、実データファイルの書き換え（→ R12g）、直接ダメージの型・schema。

**次:** [R12g](#r12g--class--module--passive-データ再設計)。

### R12g — class / module / passive データ再設計

**ゴール:** R12f の分配に従い、`classes` / CombatModule / 作戦内パッシブ候補の **データを再設計・入力**する。鉄衛士 M2 等の Hit トリガー効果は **先行して damage event 基盤**（R12g-b）を確定する。

**分割（設計・実装順）:**

| ID | 内容 | 状態 |
| -- | ---- | ---- |
| **R12g-a** | 現行 schema / effect / targeting / damage event 初回調査 | **完了** |
...
**再判定（2026-07-17）:** cost `1 / 10 / 20` 等の初回結果は characterization として維持するが、旧 Lv パッシブ候補と固定 Stage だけを前提にした最終バランスではない。R12l〜m 後の本調整は R12n で行う。
...
**状態:** **判定無効 / 未完了。** 標準Moduleは12点分散×2と24点温存→20点候補の両経路で実画面クリアした。一方、全員B側Module・無取得はWave 3でゲーム内366秒超の膠着となった。さらに、旧 Lv パッシブの未再編とメイン反復構造の欠落が後から判明したため、この固定 Stage の結果だけで「ゲームとして成立」と判定しない。結果は技術 characterization として残し、最終ゲートは R12o へ送る。
...
**明示的に含めない:** 「もう一周したい」という感情の最終判定（→ R13）。ただし、その評価を可能にする反復構造そのものは R12k〜m で先に作る。

### R12k — メイン反復構造の設計

**状態:** **完了**（2026-07-18・設計 Phase）。正本: [operation-loop.md §21](../spec/operation-loop.md#21-メイン反復構造r12k)。

**ゴール:** 固定 Stage を一度解くだけの試作から、プレイごとに別の問題と構築判断が生まれるメイン試作へ移るための上位仕様を確定する。

**採用方式:** シード付き・作者設計の線形 3 Wave 問題系列。敵兵科や Wave の個別無作為抽選はしない。ローグライク／ローグライトという正式名称は付けない。プレイヤー向け用語は「作戦」を継続。

**Backend 完了（設計 Phase・達成）:**

- メイン攻略と固定クエスト（チュートリアル / 検証 / 物語 / 作者指定課題 / デバッグ）の責務分離
- seed / 問題系列 / 作戦状態 / 戦闘状態の所有関係
- 同 seed 再現と異 seed 差分（系列 A＝攻撃 Module 判断、系列 B＝防護・回復 Module 判断）
- 全 Wave 情報開示と非表示項目（§17.5 維持）
- 勝利・敗北・再試行・中断の状態遷移
- 作戦内取得消去と非数値解禁優先。恒久数値成長不採用
- R12m 初期不採用（分岐マップ、ショップ、休憩、ランダム報酬、完全自動生成、エンドレス、途中セーブ等）
- R12m 用系列 A/B の概念例と最小実装契約

**Player 完了:** 設計 Phase のため、文章上のプレイ例（系列 A/B・開示・再試行）で検証済み。実装・UI は R12m。

**スコープ外:** 実装、最終報酬量、正式コンテンツ数、R12l の新パッシブ内容・ID・cost 効果。

### R12l — R5対象4兵科のパッシブ・旧active再編

**対象:** 鉄衛士 `df_guardian`、剣術士 `at_swordsman`、魔術師 `at_sorcerer`、療養師 `sp_cleric` のみ。

**ゴール:** 旧 `Lv0 / Lv10 / Lv20`、passive / active をそのまま新仕様として扱わず、既存効果を素材として **兵科本体 / Module A / Module B / cost 1 / cost 10 / 廃止** へ再編する。**unit 1〜2** で新 path を成立確認。**unit 3B** で旧 active / 旧 DoT・迎撃 / catalog 外 op を **物理削除**（移行待ち JSON 温存はしない）。

**Backend（unit3）:** 4兵科 isolation 完了（`r12lLegacyIsolation.test.ts`）。`emberIgnition` が種火 production 正本。

**Player（unit3）:** happy-dom までの部分確認のみ。実ブラウザ未確認。**R12l Phase Player 完了ではない。**

**確定試作量（1兵科あたり計6件）:**

| cost | 初期数 | 位置づけ |
| ---- | ------ | -------- |
| `1` | 3 | 小さな補助、穴埋め、複数取得の入口 |
| `10` | 2 | 構築方向を変える中核 |

**今回 scope 外:** cost 20、前提 / 排他 / 同兵科 cost 逓増、候補のランダム非表示。`fixedCostByPassiveId` を正本にし、4兵科の新 path では `unlockLevel` を使わない。

初期試作では候補をランダムに隠さず、**全候補を表示**して効果と取得判断を検証する。方向承認後の拡張目標は cost 1 が 5〜10、cost 10 が 2〜5、cost 20 は必要なら後続フェーズで再検討する。残り11兵科への展開は、4兵科の方向承認後に別フェーズで行う。

**Backend 完了:** 効果、担当レイヤ、Moduleとの関係、cost、再利用元 / 廃止理由、名称を一覧化し、Lv 解放へ依存しないデータとして validate / authoring 可能。

**Player 完了:** 初期準備 / Wave 間準備で4兵科の全候補、cost、Moduleとの関係、取得済み状態を読み分けられる。

### R12m — 4兵科だけの反復可能メイン試作

**ゴール:** R12k の上位構造と R12l のパッシブを、R5対象4兵科だけで一周できる最小プレイへ接続する。`r12_prototype` 1本の固定内容を再生するだけでは完了しない。**系列 A（単壁突破→複数処理）と系列 B（集中被害→分散被害）の 2 問題系列**を実装する（[operation-loop.md §21.10](../spec/operation-loop.md#2110-r12m用の-2-問題系列概念例)）。

**状態（2026-07-21・作業単位 2R1 文書同期）:**

| 層 | 判定 |
| -- | ---- |
| **Backend（1B / 1C）** | **完了** — catalog / seed / load / validation + 選出済み系列の GameSession / OperationState / BattleEngine 接続 |
| **Player production DOM** | **自動経路成立**（happy-dom）。入口→概要→3 Wave 完走→結果→同 seed / 新 seed 再開始まで。実ブラウザ未確認 |
| **2Q1 自動ゲート監査** | production 不足あり（Wave 間 §21.5 開示・4 兵科 Player 編成制限）。テスト証拠不足あり（`BattleView.test.ts` 11 失敗等） |
| **Phase 全体** | **未完了** |
| **R12n / R12o / R13** | 未着手 / 未着手 / 開始条件未達 |

**Backend 完了（実装済み）:**

- 問題系列 catalog / `generatorVersion` / seed normalize / FNV-1a deterministic 選出 / 系列 A・B / 3 Wave validation / 4 兵科制限 / CombatModule 相互参照 / class pool 所属検証
- load / normalize / round-trip / Editor 保存前 validation / 解決済み Wave 変換 / 作戦開始 snapshot / BattleEngine resolved-waves provider
- OperationSource 分離 / OperationState / checkpoint / retry / abort / final victory / fixed Stage 分離 / Save 非統合

**Player production DOM 自動経路（happy-dom・実ブラウザ未確認）:**

- 固定クエスト入口 / メイン攻略入口 / seed 入力 / 全 3 Wave 概要 / 正解を直接教えない開示 / 概要 confirm / 初期 formation
- Wave 0 戦闘 / Wave 1 準備・戦闘 / Wave 2 準備・戦闘 / 作戦内パッシブ取得 / 作戦ポイント / Wave 間引継ぎ / 最終勝利
...
**Player 完了（Phase 条件・未達）:** 複数回開始して、提示問題・取得判断・有効編成のいずれかが実際に変わり、その差を開始前 / 選択前情報から読める。**実ブラウザ確認待ち。** 2Q1 で Wave 間 §21.5 開示・4 兵科 Player 編成制限が未達。

**公式次:** **R12m 実ブラウザ確認** — 系列 A（`fixture-a`）で作戦選択→概要→3 Wave 完走→結果→同 seed / 新 seed 再開始まで手動確認。

### R12n — 新構造後の数値強度調整

**ゴール:** R12l〜m の完成後に、cost、付与量、出現頻度、敵scale、基礎stat、CombatModule / パッシブ強度を調整する。R12i の初回値は比較材料に留める。

**Backend 完了:** 複数 seed / 問題系列で、即死・無限膠着・選択無効・単一正解化を検出できる比較と回帰がある。

**Player 完了:** cost 1 の積み重ね、cost 10 の中核、滅多に取れない cost 20 の最終形が、それぞれ異なる判断として成立する。

### R12o — 反復試作の手元プレイ成立ゲート

**ゴール:** R12m のメイン試作を手元で複数回遊び、R13 の感情評価を行える水準のゲームになったか判定する。

**Player 完了（すべて満たす）:**

1. 1回を開始し、情報を読んで編成・Module・パッシブを判断し、結果まで到達できる
2. 異なる問題系列で、前回の解答をそのまま使うだけではない判断が少なくとも1つ生じる
3. 失敗が有限時間で収束し、原因を読んで再試行できる
4. 4兵科だけでも複数の有効な構築方針がある
5. legacy Lv / active / 固定 Stage 前提が新仕様の理解を混乱させない

**明示的に含めない:** 「楽しい」「もう一周したい」の最終評価。これは R13 で行う。

### R12 スコープ外（共通）

R12k〜o の共通スコープ外は、残り11兵科へのパッシブ展開、診断基盤再構築、戦場移動 legacy cleanup、Stage 削除、正式画像 / VFX / 効果音、i18n、packaging、itch.io、作戦途中セーブ。R12m 初期不採用（分岐マップ、ショップ、休憩、ランダム報酬、完全な敵編成自動生成、エンドレス、恒久数値成長等）は [operation-loop.md §21.8](../spec/operation-loop.md#218-r12m初期不採用)。採用方式の正本は同書 §21。

---

## R13 — 反復評価（「繰り返し遊びたいか」）

**状態:** **未着手**。**R12o Player 完了後**。

**評価対象先行ゲート:** 発生機構 = seed により異なる作者設計問題系列を供給するメイン攻略、設計 = R12k（完了）、実装 = R12m、成立 = R12o、評価 = R13。R12l は構築候補、R12n は強度を成立させる補助。R12o Player 未完了なら R13 を開始しない。

**反証例:** 2回目も同じ固定 Stage・同じ問題系列・同じ取得候補だけが提示され、プレイヤーが任意に別解を選ぶ以外の差がない場合、メイン反復性の評価対象は未成立。

**目的:** 本来 R10 が担うはずだった評価を、**ゲームとして遊べる試作の上で人間の Player が**行う。LLMや自動テストは面白さを完了判定しない。

> 編成、戦闘方式、Wave 間変更、作戦内パッシブを使って、異なる問題系列が提示されるメイン攻略をもう一度始めたいと思えるか。
...
- 作戦内パッシブの取得が次 Wave の編成・方式判断に影響するか
- 初回失敗後に別案を試したくなるか
- 問題系列や取得候補の違いが、次の1回を始める理由になっているか
...
R12e 必要能力・対処能力の導出（完了）
  ↓
R12f 兵科・module・パッシブへの分配（完了）
  ↓
R12g class / module / passive データ再設計
  ↓
R12h Stage / Wave データ実装
  ↓
R12i 数値強度調整（固定 Stage の初回 characterization）
  ↓
R12j 固定 Stage 手元ゲート（判定無効 / 未完了）
  ↓
R12k メイン反復構造の設計（完了）
...
- **R12m 進行中** — 1B / 1C Backend 完了 + Player production DOM 自動経路成立（happy-dom。実ブラウザ未確認）。2Q1 production 不足あり。Phase 未完了
- 正式順序は **R12k（完了）→ R12l（Backend 隔離済 / Player 部分）→ R12m（進行中）→ R12n → R12o → R13**
