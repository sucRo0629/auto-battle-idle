...
**現在地:** **R5〜R8 Backend 完了**、**R9a 完了**、**R9b 完了**（Stage enemyGroups CombatModule 編集 UI）、**R9c 完了**（複数 Wave・enemyGroups 構造 authoring）、**R9d 完了**（作戦内パッシブ候補・付与条件 authoring）、**R9e 完了**（preview・validation・参照整合の統合）、**R9g 完了**（効果範囲 authoring）、**R9.5a〜b Player 完了**、**R9.5c Backend 完了**（R5〜R8 縦切り・暫定 UI 配線確認）。**作戦準備の正式 Player UI（CombatModule・作戦内パッシブ）は R9.6 へ**。**公式次タスク: R9h**（class 方式 pool）。詳細は [current-task.md §91](../ai-handoff/current-task.md)。
...
| **R9b〜h** | 新仕様 authoring 完成 — Stage / Wave / 敵方式 / 作戦内パッシブ / validate / **効果範囲** / **class 方式 pool** / closure | **R9b〜e・R9g 完了**（§87〜91） / R9h 未着手 | R10 用作戦反映は未確認 | R9g Tooling 完了 |
...
**次タスク（R9 系列）:** **R9h** — Class 方式 pool（`combatModuleIds`）。
...
**当面のフェーズ順:** **R9h** → R9f → **R9.6（A→B）** → R10。R9.6 は R9b とは別 Phase（Player UI）。R9b〜**R9e**・**R9g** は **完了**（§87〜91）。R9.6（Player 準備 UI）は未着手。
