...
| **R8** | 作戦内パッシブ — リソース消費・任意取得・巻き戻し、**戦闘中表示整理**、**範囲パッシブ runtime 判定 + プレースホルダ範囲描画**（正式 VFX は後続） | **R8a 完了** → R8b 次 |
...
| **R8b** | `OperationState` + checkpoint — slot 別取得パッシブ・`unspentResource` の snapshot / restore / retry 整合 | テストのみ |
| **R8c** | Wave 間準備 UI — passive 直接選択 + 固定コスト取得（module 選択と共存） | Wave 間で取得→確定 |
| **R8d** | 戦闘開始注入 — operation passive → `learnedPassiveIds` マージ。**最小縦切り:** 既存 1 兵科 + self stat buff 1 個 | 1 Wave で効果確認 |
| **R8e** | 戦闘中表示 — 常時 stat 非表示・条件付きのみアイコン・HUD read-only 一覧 | 戦闘 HUD 目視 |
| **R8f** | 範囲系 runtime 判定 + 1 次元プレースホルダ描画 | フィールド帯表示 |

**次タスク:** **R8b** — 作戦内パッシブ状態モデル + checkpoint。
