...
| **R12l** | **R5対象4兵科のパッシブ・旧active再編** — 初期は各兵科 cost 1×3 / 10×2 / 20×2、全候補表示 | **次** |
...
**ゴール:** 旧 `Lv0 / Lv10 / Lv20`、passive / active をそのまま新仕様として扱わず、既存効果を素材として **兵科本体 / Module A / Module B / cost 1 / cost 10 / cost 20-A / cost 20-B / 廃止候補**へ再編する。
...
| `10` | 2 | 構築方向を変える中核 |
| `20` | 2 | 各 CombatModule の最終形を1件ずつ。非常に強く、滅多に取れなくてよい |

初期試作では候補をランダムに隠さず、**全候補を表示**して効果と取得判断を検証する。方向承認後の拡張目標は cost 1 が 5〜10、cost 10 が 2〜5、cost 20 が 1〜2。残り11兵科への展開は、4兵科の方向承認後に別フェーズで行う。
