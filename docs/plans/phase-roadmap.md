...
**現在地:** **R12g-a Backend（調査）完了 / R12g-b Backend（設計）完了 / R12g-b1 Backend 完了**（`DamageAppliedEvent` emission 統一。handoff: [current-task.md §105](../ai-handoff/current-task.md)）。公式次は **R12g-b2**（鉄衛士 M2 runtime）。ゲームとしての成立は **R12j** まで未達。反復評価は **R13**。
...
| **R12** | 試作をゲームにする — Stage 先行の敵問題設計 → 能力導出・分配 → データ実装 → 手元成立 | R12a〜f 設計完了 / R12g-a〜b1 完了 | R12a〜f 設計 Phase 完了。ゲーム成立は **R12j** | **R12g-b2 へ** |
